...
templateUnsafeNth =
  ...
  in  defineTemplate
        ...
        (FuncTy [RefTy (StructTy concreteArray [t]) (VarTy "q"), IntTy]
                (RefTy t (VarTy "q"))
                StaticLifetimeTy
        )
...
templateLength = defineTypeParameterizedTemplate templateCreator path t docs
 where
  ...
  t    = FuncTy [RefTy (StructTy concreteArray [VarTy "t"]) (VarTy "q")]
                IntTy
                StaticLifetimeTy
...
deleteTy typeEnv env (StructTy _ [innerType]) =
  ...
  , TokC $ "    " ++ ArrayTemplates.insideArrayDeletion typeEnv
                                                        env
                                                        innerType
                                                        "i"
  , TokC "    }\n"
...
templateAsetBang = defineTypeParameterizedTemplate templateCreator path t docs
 where
  ...
  t    = FuncTy
    [RefTy (StructTy concreteArray [VarTy "t"]) (VarTy "q"), IntTy, VarTy "t"]
    UnitTy
    StaticLifetimeTy
  docs =
...
templateStrArray = defineTypeParameterizedTemplate templateCreator path t docs
 where
  ...
  t    = FuncTy [RefTy (StructTy concreteArray [VarTy "a"]) (VarTy "q")]
                StringTy
                StaticLifetimeTy
