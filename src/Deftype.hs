  ...
  = let
    ...
    in
      do
        ...
        (okMembers, membersDeps) <- templatesForMembers typeEnv
                                                        env
                                                        insidePath
                                                        structTy
                                                        rest
        okInit           <- binderForInit insidePath structTy rest
        (okStr, strDeps) <- binderForStrOrPrn typeEnv
                                              env
                                              insidePath
                                              structTy
                                              rest
        ...
        (okPrn, _) <- binderForStrOrPrn typeEnv
                                        ...
                                        rest
                                        "prn"
        (okDelete, deleteDeps) <- binderForDelete typeEnv
                                                  env
                                                  insidePath
                                                  structTy
                                                  rest
...
bindingsForRegisteredType typeEnv env pathStrings typeName rest i existingEnv =
  ...
  in
    do
      ...
      (binders, deps) <- templatesForMembers typeEnv
                                             env
                                             insidePath
                                             structTy
                                             rest
      ...
      let moduleEnvWithBindings =
            addListOfBindings typeModuleEnv (okInit : okStr : okPrn : binders)
          typeModuleXObj = XObj (Mod moduleEnvWithBindings) i (Just ModuleTy)
...
templatesForMembers typeEnv env insidePath structTy [XObj (Arr membersXobjs) _ _]
  = let bindersAndDeps = concatMap
          (templatesForSingleMember typeEnv env insidePath structTy)
          (pairwise membersXobjs)
    in  Right (map fst bindersAndDeps, concatMap snd bindersAndDeps)
templatesForMembers _ _ _ _ _ =
  error "Shouldn't reach this case (invalid type definition)."

-- | Generate the templates for a single member in a deftype declaration.
templatesForSingleMember
  :: TypeEnv
  -> Env
  -> [String]
  -> Ty
  ...
  = let
    ...
    in
      [ instanceBinderWithDeps
        (SymPath insidePath memberName)
        (FuncTy [RefTy p (VarTy "q")] (RefTy t (VarTy "q")) StaticLifetimeTy)
        (templateGetter (mangle memberName) t)
        ("gets the `" ++ memberName ++ "` property of a `" ++ typeName ++ "`.")
      , if isTypeGeneric t
        then (templateGenericSetter insidePath p t memberName, [])
        else instanceBinderWithDeps
          (SymPath insidePath ("set-" ++ memberName))
          (FuncTy [p, t] p StaticLifetimeTy)
          (templateSetter typeEnv env (mangle memberName) t)
          ("sets the `" ++ memberName ++ "` property of a `" ++ typeName ++ "`."
          )
      , if isTypeGeneric t
        then (templateGenericMutatingSetter insidePath p t memberName, [])
          ...
          (templateMutatingSetter typeEnv env (mangle memberName) t)
          (  "sets the `"
          ++ memberName
          ++ "` property of a `"
          ++ typeName
      ...
      , instanceBinderWithDeps
        ...
        (FuncTy [p, RefTy (FuncTy [t] t (VarTy "fq")) (VarTy "q")]
                p
                StaticLifetimeTy
        )
...
templateGetter member memberTy = Template
...
templateSetter typeEnv env memberName memberTy =
  ...
  in  Template
        ...
        (\_ -> if
          | isManaged typeEnv memberTy -> depsOfPolymorphicFunction
            typeEnv
            env
            []
            "delete"
            (typesDeleterFunctionType memberTy)
...
templateGenericSetter pathStrings originalStructTy@(StructTy (ConcreteNameTy typeName) _) memberTy memberName
  = defineTypeParameterizedTemplate
    templateCreator
    path
    (FuncTy [originalStructTy, memberTy] originalStructTy StaticLifetimeTy)
    docs
 where
  ...
  templateCreator = TemplateCreator $ \typeEnv env -> Template
    ...
    (\(FuncTy [_, memberTy] _ _) -> if isManaged typeEnv memberTy
...
templateUpdater member = Template
  (FuncTy
    [VarTy "p", RefTy (FuncTy [VarTy "t"] (VarTy "t") (VarTy "fq")) (VarTy "q")]
    (VarTy "p")
    StaticLifetimeTy
  )
  ...
  (const
    (toTemplate
      (unlines
        ...
        ++ member
        ++ " = "
        ++ templateCodeForCallingLambda
             "(*updater)"
             (FuncTy [VarTy "t"] (VarTy "t") (VarTy "fq"))
             ["p." ++ member]
        ++ ";"
        , "    return p;"
        , "}\n"
...
initArgListTypes xobjs =
  map (\(_, x) -> fromJust (xobjToTy x)) (pairwise xobjs)
...
concreteInit allocationMode originalStructTy@(StructTy (ConcreteNameTy typeName) typeVariables) membersXObjs
  = Template
    (FuncTy (map snd (memberXObjsToPairs membersXObjs))
            (VarTy "p")
            StaticLifetimeTy
    )
    (\(FuncTy _ concreteStructTy _) ->
      let mappings = unifySignatures originalStructTy concreteStructTy
          correctedMembers =
              replaceGenericTypeSymbolsOnMembers mappings membersXObjs
          memberPairs = memberXObjsToPairs correctedMembers
      in  (  toTemplate
          $  "$p $NAME("
          ++ joinWithComma (map memberArg memberPairs)
          ++ ")"
          )
    )
...
genericInit allocationMode pathStrings originalStructTy@(StructTy (ConcreteNameTy typeName) _) membersXObjs
 ...
 where
  ...
  t    = FuncTy (map snd (memberXObjsToPairs membersXObjs))
                originalStructTy
                StaticLifetimeTy
...
tokensForInit allocationMode typeName membersXObjs = toTemplate $ unlines
  [ "$DECL {"
 ...
 where
  ...
  assignments xobjs =
    joinLines
      $   memberAssignment allocationMode
      .   fst
      <$> memberXObjsToPairs xobjs
...
templatizeTy (FuncTy argTys retTy ltTy) =
  FuncTy (map templatizeTy argTys) (templatizeTy retTy) (templatizeTy ltTy)
    ...
    then Right (genericStr insidePath structTy membersXObjs strOrPrn, [])
      ...
      (instanceBinderWithDeps
        ...
        (concreteStr typeEnv
                     env
                     structTy
                     (memberXObjsToPairs membersXObjs)
                     strOrPrn
        )
...
memberAssignment allocationMode memberName =
  "    instance" ++ sep ++ memberName ++ " = " ++ memberName ++ ";"
 where
...
genericDelete pathStrings originalStructTy@(StructTy (ConcreteNameTy typeName) _) membersXObjs
 ...
 where
  ...
  docs =
    "deletes a `" ++ typeName ++ "`. Should usually not be called manually."
  templateCreator = TemplateCreator $ \typeEnv env -> Template
    t
