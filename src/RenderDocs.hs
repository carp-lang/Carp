...
beautifyType t =
  let tys      = List.nub (typeVariablesInOrderOfAppearance t)
      mappings = Map.fromList
        (List.zip (List.map (\(VarTy name) -> name) tys)
                  (List.map (VarTy . (: [])) ['a' ..])
        )
...
saveDocsForEnvs ctx pathsAndEnvBinders =
  let
    ...
    allEnvNames   = fmap (getModuleName . fst . getEnvAndMetaFromBinder . snd)
                         pathsAndEnvBinders
  in
    do
      ...
      when
        generateIndex
        (writeFile (dir </> title ++ "_index.html")
...
getEnvAndMetaFromBinder envBinder = case envBinder of
  Binder meta (XObj (Mod env) _ _) -> (env, meta)
  _ ->
    error
      "Binder's not a module. This should be detected in 'commandSaveDocsInternal'."
...
saveDocsForEnvBinder ctx moduleNames (pathToEnv, envBinder) = do
  let SymPath _ moduleName = pathToEnv
      dir                  = projectDocsDir ctx
      fullPath             = dir </> moduleName ++ ".html"
      string =
        renderHtml (envBinderToHtml envBinder ctx (show pathToEnv) moduleNames)
...
envBinderToHtml envBinder ctx moduleName moduleNames =
  let
    (env, meta)       = getEnvAndMetaFromBinder envBinder
    title             = projectTitle ctx
    css               = projectDocsStyling ctx
    ...
    moduleDescriptionHtml =
      commonmarkToHtml [optSafe] $ Text.pack moduleDescription
  in
    H.docTypeHtml $ do
      headOfPage css
      H.body $ H.div ! A.class_ "content" $ do
        H.div ! A.class_ "logo" $ do
          H.a ! A.href "http://github.com/carp-lang/Carp" $ H.img ! A.src
            "logo.png"
          --span_ "CARP DOCS FOR"
          H.div ! A.class_ "title" $ toHtml title
          moduleIndex moduleNames
        H.h1 (toHtml moduleName)
        H.div ! A.class_ "module-description" $ preEscapedToHtml
          moduleDescriptionHtml
        mapM_
          (binderToHtml . snd)
          (Prelude.filter shouldEmitDocsForBinder (Map.toList (envBindings env))
          )
...
moduleIndex moduleNames =
  H.div ! A.class_ "index" $ H.ul $ mapM_ moduleLink moduleNames
...
binderToHtml (Binder meta xobj) =
  ...
  in  H.div ! A.class_ "binder" $ do
        H.a
          ! A.class_ "anchor"
          ! A.href (stringValue ("#" ++ name))
          $ H.h3
          ! A.id (stringValue name)
          $ toHtml name
