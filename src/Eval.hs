...
eval ctx xobj@(XObj o i t) = case o of
  Lst body -> eval' body
  Sym path@(SymPath p n) _ ->
    case lookupInEnv (SymPath ("Dynamic" : p) n) (contextGlobalEnv ctx) of
      ...
      Nothing                  -> if null p
        then case tryInternalLookup path of
          Just v  -> return v
          Nothing -> tryLookup path
        else tryLookup path
   where
    tryInternalLookup path = case contextInternalEnv ctx of
      Nothing -> Nothing
      Just e  -> case lookupInEnv path e of
        Just (_, Binder _ found) -> Just (ctx, Right (resolveDef found))
        Nothing                  -> Nothing
    tryLookup path = case lookupInEnv path (contextGlobalEnv ctx) of
      Just (_, Binder _ found) -> return (ctx, Right (resolveDef found))
      Nothing -> case lookupInEnv path (getTypeEnv (contextTypeEnv ctx)) of
        Just (_, Binder _ found) -> return (ctx, Right (resolveDef found))
        Nothing ->
 ...
 where
  ...
  eval' form = case form of
    ...
    [XObj (Defn _) _ _, name, args@(XObj (Arr a) _ _), body] ->
      case obj name of
        (Sym (SymPath [] _) _) -> if all isUnqualifiedSym a
    ...
    [XObj (Defn _) _ _, name, invalidArgs, _] -> return
      (evalError
        ctx
    ...
    (defn@(XObj (Defn _) _ _) : _) -> return
      (evalError
        ctx
        (  "I didn’t understand the `defn` at "
    ...
    [def@(XObj Def _ _), name, expr] -> if isUnqualifiedSym name
      then specialCommandDefine ctx xobj
      else return
        (evalError
          ctx
    ...
    [the@(XObj The _ _), ty, value] -> do
      (newCtx, evaledValue) <- expandAll eval ctx value -- TODO: Why expand all here?
      return
        ( newCtx
        , do
          okValue <- evaledValue
    ...
    (XObj The _ _ : _) -> return
      (evalError
        ctx
        (  "I didn’t understand the `the` at "
        ++ prettyInfoFromXObj xobj
        ++ ":\n\n"
        ++ pretty xobj
    ...
    [XObj Let _ _, XObj (Arr bindings) bindi bindt, body]
      | odd (length bindings) -> return
        (evalError ctx
                   ("Uneven number of forms in `let`: " ++ pretty xobj)
                   (info xobj) -- Unreachable?
        )
      | not (all isSym (evenIndices bindings)) -> return
        (evalError
          ctx
      ...
      | otherwise -> do
        ...
        eitherCtx <- foldrM successiveEval
                            (Right ctx { contextInternalEnv = Just ni })
                            binds
        case eitherCtx of
     ...
     where
      unwrapVar [] acc = acc
      unwrapVar ((XObj (Sym (SymPath [] x) _) _ _, y) : xs) acc =
        unwrapVar xs ((x, y) : acc)
      successiveEval (n, x) = \case
        ...
        Right ctx    -> do
          ...
          case res of
            Right okX -> do
              ...
              return $ Right
                (newCtx
                  { contextInternalEnv = Just
                                           (envInsertAt e (SymPath [] n) binder)
                  }
                )
            Left err -> return $ Left err
    l@[XObj Fn{} _ _, args@(XObj (Arr a) _ _), f] -> if all isUnqualifiedSym a
      then return
        (ctx, Right (XObj (Closure (XObj (Lst l) i t) (CCtx ctx)) i t))
      else return
        (evalError
          ctx
          ("`fn` requires all arguments to be unqualified symbols, but it got `"
          ++ pretty args
          ++ "`"
          )
          (info args)
    ...
    XObj (Lst [XObj (Command callback) _ _, _]) _ _ : args -> do
      (newCtx, evaledArgs) <- foldlM successiveEval (ctx, Right []) args
      case evaledArgs of
        Right okArgs -> getCommand callback ctx okArgs
        Left  err    -> return (ctx, Left err)
    x@(XObj (Lst [XObj (Primitive prim) _ _, _]) _ _) : args ->
      (getPrimitive prim) x ctx args
    XObj (Lst (XObj (Defn _) _ _ : _)) _ _ : _ -> return (ctx, Right xobj)
    XObj (Lst (XObj (Interface _ _) _ _ : _)) _ _ : _ ->
      return (ctx, Right xobj)
    XObj (Lst (XObj (Instantiate _) _ _ : _)) _ _ : _ ->
      return (ctx, Right xobj)
    XObj (Lst (XObj (Deftemplate _) _ _ : _)) _ _ : _ ->
      return (ctx, Right xobj)
    ...
    l@(XObj (Lst _) i t) : args -> do
      ...
      case f of
        Right fun -> do
          (newCtx', res) <- eval (pushFrame newCtx xobj)
                                 (XObj (Lst (fun : args)) i t)
    ...
    x@(XObj sym@(Sym s _) i _) : args -> do
      ...
      case f of
        Right fun -> do
          (newCtx', res) <- eval (pushFrame ctx xobj)
                                 (XObj (Lst (fun : args)) i t)
          return (popFrame newCtx', res)
        Left err -> return (newCtx, Left err)
    XObj With _ _ : xobj@(XObj (Sym path _) _ _) : forms ->
      specialCommandWith ctx xobj path forms
    XObj With _ _ : _ -> return
      (evalError ctx
                 ("Invalid arguments to `with`: " ++ pretty xobj)
                 (info xobj)
      )
    XObj SetBang _ _ : args -> specialCommandSet ctx args
    [XObj Do _ _] -> return (evalError ctx "No forms in do" (info xobj))
    XObj Do _ _ : rest -> foldlM successiveEval (ctx, dynamicNil) rest
     where
      successiveEval (ctx, acc) x = case acc of
        err@(Left _) -> return (ctx, err)
        Right _      -> eval ctx x
    [XObj While _ _, cond, body] -> specialCommandWhile ctx cond body
    []                           -> return (ctx, dynamicNil)
    x                            -> do
      return
        (evalError ctx
                   ("I did not understand the form `" ++ pretty xobj ++ "`")
                   (trace (show xobj) (info xobj))
        )
...
macroExpand ctx xobj = case xobj of
  ...
  _ -> return (ctx, Right xobj)
 where
  successiveExpand (ctx, acc) x = case acc of
    Left  _ -> return (ctx, acc)
    Right l -> do
...
apply
  :: Context -> XObj -> [XObj] -> [XObj] -> IO (Context, Either EvalError XObj)
apply ctx@Context { contextInternalEnv = internal } body params args =
 ...
 where
  callWith env proper rest = do
    let
      n          = length proper
      insideEnv  = Env Map.empty internal Nothing [] InternalEnv 0
      insideEnv' = foldl' (\e (p, x) -> extendEnv e p x)
                          insideEnv
...
executeString doCatch printResult ctx input fileName = if doCatch
  then catch exec (catcher ctx)
  else exec
 where
  exec = case parse input fileName of
    Left parseError ->
      let sourcePos      = Parsec.errorPos parseError
          parseErrorXObj = XObj
            (Lst [])
            (Just dummyInfo { infoFile   = fileName
    ...
    Right xobjs -> do
      ...
      return ctx
  interactiveFolder (_, context) xobj = executeCommand context xobj
...
executeCommand ctx s@(XObj (Sym _ _) _ _) = executeCommand
  ctx
  (XObj
    (Lst [XObj (Sym (SymPath [] "info") Symbol) (Just dummyInfo) Nothing, s])
    (Just dummyInfo)
    Nothing
  )
executeCommand ctx@(Context env _ _ _ _ _ _ _) xobj = do
  when (isJust (envModuleName env)) $ error
    (  "Global env module name is "
    ++ fromJust (envModuleName env)
    ++ " (should be Nothing)."
    )
  ...
  case result of
    ...
    Right (XObj (Lst (XObj (Lst (XObj (Defn _) _ _ : (XObj (Sym (SymPath [] "main") _) _ _) : _)) _ _ : _)) _ _)
      -> executeCommand
        newCtx
        (withBuildAndRun (XObj (Lst []) (Just dummyInfo) Nothing))
    ...
    Right (XObj (Lst (XObj (Lst (XObj (External _) _ _ : name : _)) _ _ : args)) i _)
      -> callFromRepl newCtx (XObj (Lst (name : args)) i Nothing)
    Right x@(XObj (Lst (XObj (Match _) _ _ : _)) _ _) -> callFromRepl newCtx x
    Right x@(XObj (Lst (XObj The       _ _ : _)) _ _) -> callFromRepl newCtx x
...
reportExecutionError ctx errorMessage = case contextExecMode ctx of
  Check -> putStrLn errorMessage
  _     -> do
    putStrLnWithColor Red errorMessage
    throw CancelEvaluationException

-- | Decides what to do when the evaluation fails for some reason.
catcher :: Context -> CarpException -> IO Context
catcher ctx exception = case exception of
  (ShellOutException message returnCode) -> do
    putStrLnWithColor Red ("[RUNTIME ERROR] " ++ message)
    stop returnCode
  CancelEvaluationException -> stop 1
  EvalException evalError   -> do
    putStrLnWithColor Red (show evalError)
    stop 1
 where
  stop returnCode = case contextExecMode ctx of
    Repl        -> return ctx
...
specialCommandWith
  :: Context -> XObj -> SymPath -> [XObj] -> IO (Context, Either EvalError XObj)
specialCommandWith ctx xobj path forms = do
  let
    ...
    env'        = if path `elem` useThese
      then env
      else env { envUseModules = path : useThese }
    ctx' = ctx { contextGlobalEnv = env' }
  ctxAfter <- liftIO $ foldM folder ctx' forms
  let envAfter = contextEnv ctxAfter
      ctxAfter' =
        ctx { contextGlobalEnv = envAfter { envUseModules = useThese } } -- This will undo ALL use:s made inside the 'with'.
  return (ctxAfter', dynamicNil)

specialCommandDefine :: Context -> XObj -> IO (Context, Either EvalError XObj)
specialCommandDefine ctx xobj = do
...
getSigFromDefnOrDef
  ...
  -> FilePathPrintLength
  -> XObj
  -> (Either EvalError (Maybe (Ty, XObj)))
getSigFromDefnOrDef ctx globalEnv fppl xobj@(XObj _ i t) =
  ...
  in
    case Map.lookup "sig" (getMeta metaData) of
      Just foundSignature -> case xobjToTy foundSignature of
        Just t ->
          let
            sigToken = XObj (Sym (SymPath [] "sig") Symbol) Nothing Nothing
            nameToken =
              XObj (Sym (SymPath [] (getName xobj)) Symbol) Nothing Nothing
            recreatedSigForm = XObj
              (Lst [sigToken, nameToken, foundSignature])
              Nothing
              (Just MacroTy)
          in
            Right (Just (t, recreatedSigForm))
        Nothing -> Left
          (EvalError
            ("Can't use '" ++ pretty foundSignature ++ "' as a type signature")
            (contextHistory ctx)
            fppl
            (info xobj)
          )
...
annotateWithinContext
  :: Bool -> Context -> XObj -> IO (Context, Either EvalError (XObj, [XObj]))
  ...
  case sig of
    ...
    Right okSig -> do
      ...
      case expansionResult of
        ...
        Right expanded ->
          let
            xobjFullPath = if qualifyDefn
              then setFullyQualifiedDefn expanded
                                         (SymPath pathStrings (getName xobj))
              else expanded
            xobjFullSymbols =
              setFullyQualifiedSymbols typeEnv globalEnv innerEnv xobjFullPath
          in
            case annotate typeEnv globalEnv xobjFullSymbols okSig of
              Left err -> case contextExecMode ctx of
                Check ->
                  ...
                  in
                    return
                      (evalError
                        ctx
                        (joinLines (machineReadableErrorStrings fppl err))
                        Nothing
                      )
                _ -> return (evalError ctx (show err) (info xobj))
              Right ok -> return (ctx, Right ok)

primitiveDefmodule :: Primitive
primitiveDefmodule xobj ctx@(Context env i typeEnv pathStrings proj lastInput execMode history) (XObj (Sym (SymPath [] moduleName) _) _ _ : innerExpressions)
  = do
    let
      ...
      defineIt meta = do
        let
          parentEnv = getEnv env pathStrings
          innerEnv  = Env (Map.fromList [])
                          (Just parentEnv)
                          (Just moduleName)
                          []
                          ExternalEnv
          ...
          globalEnvWithModuleAdded = envInsertAt
            env
            (SymPath pathStrings moduleName)
            (Binder meta newModule)
          ctx' = Context globalEnvWithModuleAdded
                         (Just (innerEnv { envParent = i }))
                         typeEnv
                         (pathStrings ++ [moduleName])
                         proj
                         lastInput
                         execMode
                         history
        (ctxAfterModuleDef, res) <- liftIO
          $ foldM folder (ctx', dynamicNil) innerExpressions
        return (popModulePath ctxAfterModuleDef { contextInternalEnv = i }, res)

    (newCtx, result) <- case lookupInEnv (SymPath pathStrings moduleName) env of
      Just (_, Binder _ (XObj (Mod innerEnv) _ _)) -> do
        let ctx' = Context env
                           ...
                           lastInput
                           execMode
                           history -- TODO: use { = } syntax instead
        (ctxAfterModuleAdditions, res) <- liftIO
          $ foldM folder (ctx', dynamicNil) innerExpressions
        return
          ( popModulePath ctxAfterModuleAdditions { contextInternalEnv = i }
          , res
          ) -- TODO: propagate errors...
      Just (_, Binder existingMeta (XObj (Lst [XObj DocStub _ _, _]) _ _)) ->
        defineIt existingMeta
      Just (_, Binder _ x) -> return
        (evalError ctx
                   ("Can't redefine '" ++ moduleName ++ "' as module")
                   (info xobj)
        )
      Nothing -> defineIt emptyMeta

    case result of
      Left  err -> return (newCtx, Left err)
      Right _   -> return (newCtx, dynamicNil)
 where
  folder (ctx, r) x = case r of
...
primitiveDefmodule _ ctx (x : _) = return
  (evalError
    ctx
    ("`defmodule` expects a symbol, got '" ++ pretty x ++ "' instead.")
    (info x)
  )
primitiveDefmodule _ ctx [] = return
  (evalError ctx
             "`defmodule` requires at least a symbol, received none."
             (Just dummyInfo)
  )
...
loadInternal ctx xobj path i reloadMode = do
  ...
  let relativeTo = case i of
      ...
      fullSearchPaths =
        path
          :  (relativeTo </> path)
          : -- the path from the file that contains the '(load)', or the current directory if not loading from a file (e.g. the repl)
      ...
      firstM p (x : xs) = do
        q <- p x
        if q then return $ Just x else firstM p xs
        ...
        then return $ cantLoadSelf ctx path
          ...
          if canonicalPath `elem` alreadyLoaded
            then return (ctx, dynamicNil)
              ...
              newCtx <- liftIO $ executeString True
                                               False
 ...
 where
  frozenPaths proj = if projectForceReload proj
  ...
  isFrozen _      = False
  fppl ctx = projectFilePathPrintLength (contextProj ctx)
  invalidPath ctx path = evalError
    ctx
    ((case contextExecMode ctx of
       Check ->
         machineReadableInfoFromXObj (fppl ctx) xobj
  ...
  replaceC c s [] = []
  replaceC c s (a : b) =
    if a == c then s ++ replaceC c s b else a : replaceC c s b
  cantLoadSelf ctx path = case contextExecMode ctx of
    Check -> evalError
      ctx
      (  machineReadableInfoFromXObj (fppl ctx) xobj
      ++ " A file can't load itself: '"
      ++ path
      ++ "'"
      )
      (info xobj)
  ...
  fromURL url =
    ...
    in  if fst `elem` ["https_COLON_", "http_COLON_"]
          then joinWith "/" (tail (tail split))
            ...
            then joinWith "/"
                          (joinWith "@" (tail (splitOn "@" fst)) : tail split)
            else url
  tryInstallWithCheckout path toCheckout = do
    let proj = contextProj ctx
    fpath <-
      liftIO $ cachePath $ projectLibDir proj </> fromURL path </> toCheckout
    ...
    (_, txt, _) <- liftIO $ readProcessWithExitCode
      "git"
    ...
    if txt == "HEAD\n"
      then do
        ...
        _ <- liftIO
          $ readProcessWithExitCode "git" ["remote", "add", "origin", path] ""
        (x0, _, stderr0) <- liftIO
          $ readProcessWithExitCode "git" ["fetch", "--all", "--tags"] ""
        case x0 of
          ExitFailure _ -> do
            _ <- liftIO $ setCurrentDirectory cur
          ...
          ExitSuccess -> do
            (x1, _, stderr1) <- liftIO
              $ readProcessWithExitCode "git" ["checkout", toCheckout] ""
            _ <- liftIO $ setCurrentDirectory cur
            case x1 of
...
loadFilesExt loadCmd ctxStart filesToLoad = foldM folder ctxStart filesToLoad
 where
  folder :: Context -> FilePath -> IO Context
  folder ctx file = do
...
commandReload ctx args = do
  let
    paths = projectFiles (contextProj ctx)
    f :: Context -> (FilePath, ReloadMode) -> IO Context
    f context (_, Frozen) | not (projectForceReload (contextProj context)) =
      return context
    f context (filepath, _) = do
...
commandExpand ctx [xobj] = do
  ...
  case result of
    Left  e        -> return (newCtx, Left e)
    Right expanded -> liftIO $ do
      putStrLnWithColor Yellow (pretty expanded)
      return (newCtx, dynamicNil)
    ...
    Right expanded ->
      case
          annotate
            typeEnv
            globalEnv
            (setFullyQualifiedSymbols typeEnv globalEnv globalEnv expanded)
            Nothing
        of
...
printC :: XObj -> String
printC xobj = case checkForUnresolvedSymbols xobj of
  Left  e -> strWithColor Red (show e ++ ", can't print resulting code.\n")
  Right _ -> strWithColor Green (toC All (Binder emptyMeta xobj))
        ...
        , case ty xobj of
          ...
          Just _ -> XObj
            (Lst
              ...
              , XObj (Lst [XObj Ref di Nothing, xobj])    di (Just UnitTy)
              ]
            )
            di
            (Just UnitTy)
...
primitiveDefdynamic _ ctx [XObj (Sym (SymPath [] name) _) _ _, value] = do
  ...
  case result of
    Left  err        -> return (newCtx, Left err)
    Right evaledBody -> dynamicOrMacroWith
      newCtx
      (\path ->
        [ XObj DefDynamic        Nothing Nothing
      ...
      name
      value
primitiveDefdynamic _ ctx [notName, body] = return
  (evalError
    ctx
    (  "`defndynamic` expected a name as first argument, but got "
    ++ pretty notName
    )
    (info notName)
  )
  ...
  case result of
    ...
    Right evald -> do
      ...
      case contextInternalEnv nctx of
        ...
        Just env -> if contextPath nctx == mod
          then return
            ( nctx
              { contextInternalEnv = Just
                (envReplaceBinding (SymPath [] n) binder env)
              }
            , dynamicNil
            )
          else return (nctx, dynamicNil)
specialCommandSet ctx [notName, body] = return
  (evalError
    ctx
    ("`set!` expected a name as first argument, but got " ++ pretty notName)
    (info notName)
  )
specialCommandSet ctx args = return
  (evalError
    ctx
    (  "`set!` takes a name and a value, but got `"
    ++ intercalate " " (map pretty args)
    )
    (if null args then Nothing else info (head args))
  )
...
dynamicOrMacro ctx pat ty name params body = do
  ...
  case exp of
    Right expanded -> dynamicOrMacroWith
      ctx'
      (\path ->
        [ XObj pat               Nothing Nothing
        , XObj (Sym path Symbol) Nothing Nothing
        , params
        , expanded
        ]
    ...
    Right (XObj (Bol ab) _ _) -> if ab
      then do
        ...
        case evaledB of
          ...
          Right b ->
            return (evalError ctx ("Can’t call `or` on " ++ pretty b) (info b))
      else return (newCtx, Right falseXObj)
    Right a ->
      return (evalError ctx ("Can’t call `or` on " ++ pretty a) (info a))
