...
completeKeywordsAnd context word = findKeywords
  word
  (bindingNames (contextGlobalEnv context) ++ keywords)
  []
 where
  findKeywords match []       res = res
  findKeywords match (x : xs) res = if match `isPrefixOf` x
    then findKeywords match xs (res ++ [simpleCompletion x])
    else findKeywords match xs res
  keywords =
    [ "Int"
    ,
      -- we should probably have a list of those somewhere
      "Float"
    , "Double"
    , "Bool"
    , "String"
...
readlineSettings historyFile = Settings
  { complete       = completeWordWithPrev
                       Nothing
                       ['(', ')', '[', ']', ' ', '\t', '\n']
                       (\_ w -> do
                         ctx <- get
...
treatSpecialInput (':' : rest) =
 ...
 where
  makeCommand args cmd = case Map.lookup cmd specialCommands of
      ...
      case balanced of
        "" -> do
          let input' = if concatenated == "\n"
                then contextLastInput context
                else concatenated -- Entering an empty string repeats last input
          context' <- liftIO $ executeString True
                                             True
                                             (resetAlreadyLoadedFiles context)
                                             (treatSpecialInput input')
                                             "REPL"
          put context'
          repl "" (projectPrompt proj)
        _ -> repl concatenated
                  (if projectBalanceHints proj then balanced else "")
...
runRepl context = do
  ...
  runStateT
    (runInputT (readlineSettings historyFile)
