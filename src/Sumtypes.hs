...
tokensForCaseInit allocationMode sumTy@(StructTy (ConcreteNameTy typeName) typeVariables) sumtypeCase =
  toTemplate $ unlines [ "$DECL {"
                       ...
                       , unlines (map (caseMemberAssignment allocationMode correctedName)
...
strCase typeEnv env concreteStructTy@(StructTy _ typeVariables) theCase =
  ...
  in unlines
     ...
     , unlines (map (memberPrn typeEnv env) (zip (map (\anon -> name ++ "." ++ anon)
                                                       anonMemberNames) tys))
     , "    bufferPtr--;"
     , "    sprintf(bufferPtr, \")\");"
     , "  }"
...
deleteCase typeEnv env concreteStructTy@(StructTy _ typeVariables) (theCase, isFirstCase) =
  ...
  in unlines
     ...
     , unlines (map (memberDeletion typeEnv env) (zip (map (\anon -> name ++ "." ++ anon)
                                                                 anonMemberNames) tys))
     , "  }"
     ]
...
tokensForSumtypeCopy typeEnv env concreteStructTy cases =
  toTemplate $ unlines [ "$DECL {"
                       , "    $p copy = *pRef;"
                       , unlines (map (copyCase typeEnv env concreteStructTy) (zip cases (True : repeat False)))
                       , "    return copy;"
                       , "}"]
...
copyCase typeEnv env concreteStructTy@(StructTy _ typeVariables) (theCase, isFirstCase) =
  ...
  in unlines
     ...
     , unlines (map (memberCopy typeEnv env) (zip (map (\anon -> name ++ "." ++ anon) anonMemberNames) tys))
     , "    }"
     ]
