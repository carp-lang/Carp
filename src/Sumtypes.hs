...
moduleForSumtype
  ...
  -> Maybe Info
  -> Maybe Env
  -> Either TypeError (String, XObj, [XObj])
moduleForSumtype typeEnv env pathStrings typeName typeVariables rest i existingEnv
  = let
      typeModuleName = typeName
      typeModuleEnv  = fromMaybe
        (Env (Map.fromList []) (Just env) (Just typeModuleName) [] ExternalEnv 0
        )
        existingEnv
    ...
    in
      do
        ...
        (okStr, okStrDeps) <- binderForStrOrPrn typeEnv
                                                env
                                                insidePath
                                                structTy
                                                cases
                                                "str"
        (okPrn, _) <- binderForStrOrPrn typeEnv
                                        env
                                        insidePath
                                        structTy
                                        cases
        ...
        (okCopy, okCopyDeps) <- binderForCopy typeEnv
                                              env
                                              insidePath
                                              structTy
                                              cases
        okMemberDeps <- memberDeps typeEnv cases
        let moduleEnvWithBindings = addListOfBindings
              typeModuleEnv
              (okIniters ++ [okStr, okPrn, okDelete, okCopy, okTag])
            typeModuleXObj = XObj (Mod moduleEnvWithBindings) i (Just ModuleTy)
        return
          ( typeModuleName
          , typeModuleXObj
          , okMemberDeps ++ okCopyDeps ++ okStrDeps
          )

memberDeps :: TypeEnv -> [SumtypeCase] -> Either TypeError [XObj]
memberDeps typeEnv cases =
  fmap concat (mapM (concretizeType typeEnv) (concatMap caseTys cases))

replaceGenericTypesOnCases :: TypeMappings -> [SumtypeCase] -> [SumtypeCase]
replaceGenericTypesOnCases mappings cases = map replaceOnCase cases
 where
...
initers
  :: [String] -> Ty -> [SumtypeCase] -> Either TypeError [(String, Binder)]
initers insidePath structTy cases =
  mapM (binderForCaseInit insidePath structTy) cases
...
concreteCaseInit
  :: AllocationMode -> [String] -> Ty -> SumtypeCase -> (String, Binder)
concreteCaseInit allocationMode insidePath structTy sumtypeCase =
  instanceBinder (SymPath insidePath (caseName sumtypeCase))
                 (FuncTy (caseTys sumtypeCase) structTy StaticLifetimeTy)
                 template
                 doc
 where
  ...
  template = Template
    ...
    (\(FuncTy _ concreteStructTy _) ->
      ...
      in  (  toTemplate
          $  "$p $NAME("
          ++ joinWithComma
               (zipWith (curry memberArg) anonMemberNames correctedTys)
          ++ ")"
          )
    )
...
genericCaseInit allocationMode pathStrings originalStructTy sumtypeCase =
 ...
 where
  ...
  templateCreator = TemplateCreator $ \typeEnv env -> Template
    (FuncTy (caseTys sumtypeCase) (VarTy "p") StaticLifetimeTy)
    (\(FuncTy _ concreteStructTy _) ->
    ...
    (\(FuncTy _ concreteStructTy _) ->
      case concretizeType typeEnv concreteStructTy of
        Left err -> error
          (show err
          ++ ". This error should not crash the compiler - change return type to Either here."
          )
        Right ok -> ok
    )
...
tokensForCaseInit allocationMode sumTy@(StructTy (ConcreteNameTy typeName) typeVariables) sumtypeCase
  = toTemplate $ unlines
    ...
    , joinLines
    $   caseMemberAssignment allocationMode correctedName
    .   fst
...
binderForTag insidePath originalStructTy@(StructTy (ConcreteNameTy typeName) _)
 ...
 where
  ...
  template = Template
    ...
    (\(FuncTy [RefTy structTy _] IntTy _) ->
      toTemplate $ proto structTy ++ " { return p->_tag; }"
    )
    (\_ -> [])
...
binderForStrOrPrn
  ...
  -> [SumtypeCase]
  -> String
  -> Either TypeError ((String, Binder), [XObj])
...
concreteStr typeEnv env insidePath concreteStructTy@(StructTy (ConcreteNameTy typeName) _) cases strOrPrn
 ...
 where
  ...
  template = Template
    (FuncTy [RefTy concreteStructTy (VarTy "q")] StringTy StaticLifetimeTy)
    (\(FuncTy [RefTy structTy _] StringTy _) ->
      toTemplate $ "String $NAME(" ++ tyToCLambdaFix structTy ++ " *p)"
    )
    ...
    (\ft@(FuncTy [RefTy structTy@(StructTy _ concreteMemberTys) _] StringTy _) ->
      concatMap
        ( depsOfPolymorphicFunction typeEnv env [] "prn"
        . typesStrFunctionType typeEnv
        )
        (filter
          (\t ->
            (not . isExternalType typeEnv) t && (not . isFullyGenericType) t
          )
          (concatMap caseTys cases)
        )
    )
 ...
 where
  ...
  templateCreator = TemplateCreator $ \typeEnv env -> Template
    ...
    (\ft@(FuncTy [RefTy concreteStructTy@(StructTy _ concreteMemberTys) _] StringTy _) ->
      let
        mappings       = unifySignatures originalStructTy concreteStructTy
        correctedCases = replaceGenericTypesOnCases mappings cases
        tys            = filter
          (\t ->
            (not . isExternalType typeEnv) t && (not . isFullyGenericType) t
      ...
      in
        concatMap
            ...
            tys
          ++ (if isTypeGeneric concreteStructTy
              then
                []
              else
...
tokensForStr typeEnv env typeName cases concreteStructTy = toTemplate $ unlines
  [ "$DECL {"
...
binderForDelete
  ...
  -> Ty
  -> [SumtypeCase]
  -> Either TypeError (String, Binder)
...
genericSumtypeDelete pathStrings originalStructTy cases =
  defineTypeParameterizedTemplate
    templateCreator
    path
    (FuncTy [originalStructTy] UnitTy StaticLifetimeTy)
    docs
 where
  ...
  templateCreator = TemplateCreator $ \typeEnv env -> Template
    ...
    (\(FuncTy [concreteStructTy] UnitTy _) ->
      ...
      in  (toTemplate $ unlines
            [ "$DECL {"
            , concatMap (deleteCase typeEnv env concreteStructTy)
                        (zip correctedCases (True : repeat False))
            , "}"
            ]
          )
    )
 ...
 where
  ...
  template = Template
    ...
    (const
      (toTemplate $ unlines
        [ "$DECL {"
        , concatMap (deleteCase typeEnv env structTy)
                    (zip cases (True : repeat False))
        , "}"
        ]
      )
    )
...
genericSumtypeCopy pathStrings originalStructTy cases =
  defineTypeParameterizedTemplate
    templateCreator
    path
    (FuncTy [RefTy originalStructTy (VarTy "q")]
            originalStructTy
            StaticLifetimeTy
    )
    docs
    ...
    (\(FuncTy [RefTy concreteStructTy _] _ _) ->
      ...
      in  if isTypeGeneric concreteStructTy
            then []
              ...
              ( depsOfPolymorphicFunction typeEnv env [] "copy"
...
tokensForSumtypeCopy typeEnv env concreteStructTy cases = toTemplate $ unlines
  [ "$DECL {"
  , "    $p copy = *pRef;"
  , joinLines $ map (copyCase typeEnv env concreteStructTy)
                    (zip cases (True : repeat False))
  , "    return copy;"
  , "}"
  ]
