...
toTemplate text = case Parsec.runParser templateSyntax 0 "(template)" text of
 ...
 where
  ...
  parseTokC = do
   ...
   where
    validInSymbol = Parsec.choice
      [Parsec.letter, Parsec.digit, Parsec.oneOf validCharactersInTemplate]
  ...
  parseCharBalanced = do
    ...
    if balanceState > 0
      then Parsec.try openParen <|> Parsec.try closeParen <|> Parsec.anyChar
...
toTokTy mode s = case parse s "" of
  Left  err    -> error (show err)
  ...
  Right [xobj] -> case xobjToTy xobj of
    ...
    Nothing ->
      error
        (  "toTokTy failed to convert this s-expression to a type: "
        ++ pretty xobj
        )
  Right xobjs -> error
    (  "toTokTy parsed too many s-expressions: "
    ++ joinWithSpace (map pretty xobjs)
    )
