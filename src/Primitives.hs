        ...
        Just (_, Binder meta defobj) ->
          do
            case inter of
              ...
              Nothing ->
                do
                  putStrWithColor
                    Blue
                    ( "[WARNING] The interface " ++ show interface ++ " implemented by " ++ show impl
                        ++ " at "
            ...
            case registerDefnOrDefInInterfaceIfNeeded ctx defobj interface of
              ...
              Right ctx' ->
                do
                  currentImplementations <- primitiveMeta xobj ctx [i, XObj (Str "implements") (Just dummyInfo) (Just StringTy)]
                  case snd currentImplementations of
                    ...
                    Right old@(XObj (Lst impls) inf ty) ->
                      let newImpls =
                       ...
                       in return
                            ( ctx' {contextGlobalEnv = envInsertAt global (getPath defobj) (Binder adjustedMeta defobj)},
                              dynamicNil
                            )
                    _ ->
            ...
            -- thus we pass inter to all three xobj positions.
              ...
              if isRight $ solve [Constraint interfaceSignature definitionSignature inter inter inter OrdInterfaceImpl]
                then
                  ...
                  Left
                    ( "[INTERFACE ERROR] " ++ show path ++ " : " ++ show definitionSignature
          ...
          do
              ...
              Just previousTypeUnwrapped ->
                unless (areUnifiable (forceTy annXObj) previousTypeUnwrapped) $
                  do
                    putStrWithColor
                      Blue
                      ( "[WARNING] Definition at " ++ prettyInfoFromXObj annXObj ++ " changed type of '" ++ show (getPath annXObj)
                          ++ "' from "
            ...
            case Map.lookup "implements" (getMeta previousMeta) of
              Just (XObj (Lst interfaces) _ _) ->
                do
                  let result = foldM (\ctx (xobj, interface) -> registerDefnOrDefInInterfaceIfNeeded ctx xobj interface) ctx (zip (cycle [annXObj]) (map getPath interfaces))
                  case result of
                    Left err ->
                      do
                        case contextExecMode ctx of
...
primitiveDeftype xobj ctx (name : rest) =
  case rest of
    (XObj (Arr a) _ _ : _) ->
      if all isUnqualifiedSym (map fst (members a))
        then deftype name
        else
          return
            ( makeEvalError
          ...
          case creatorFunction typeEnv env pathStrings typeName okTypeVariables rest i preExistingModule of
            Right (typeModuleName, typeModuleXObj, deps) ->
              let structTy = StructTy (ConcreteNameTy typeName) okTypeVariables
                  ...
                  ctx' =
                    ( ctx
                        { contextGlobalEnv = envInsertAt env (SymPath pathStrings typeModuleName) (Binder existingMeta typeModuleXObj),
