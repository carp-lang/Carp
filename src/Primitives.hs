...
found ctx binder = liftIO $ do
  putStrLnWithColor White (show binder)
...
argumentErr ctx fun ty number actual = return
  (evalError
    ctx
    (  "`"
    ++ fun
...
makePrim'
  :: String -> Maybe Int -> String -> String -> Primitive -> (String, Binder)
makePrim' name maybeArity docString example callback =
  let
    ...
    prim = XObj
      ...
      (Just DynamicTy)
    meta =
      MetaData (Map.insert "doc" (XObj (Str doc) Nothing Nothing) Map.empty)
  in
    (name, Binder meta prim)
 where
  wrapped = case maybeArity of
    Just a -> \x c l ->
      let ll = length l in (if ll /= a then err x c a ll else callback x c l)
    Nothing -> callback
  err :: XObj -> Context -> Int -> Int -> IO (Context, Either EvalError XObj)
  err x ctx a l = return
    (evalError
      ctx
      (  "The primitive `"
      ++ name
...
primitiveFile x@(XObj _ i t) ctx [] = case i of
  Just info -> return (ctx, Right (XObj (Str (infoFile info)) i t))
  Nothing ->
    return (evalError ctx ("No information about object " ++ pretty x) (info x))
primitiveFile x@(XObj _ i t) ctx [XObj _ mi _] = case mi of
  Just info -> return (ctx, Right (XObj (Str (infoFile info)) i t))
  Nothing ->
    return (evalError ctx ("No information about object " ++ pretty x) (info x))
primitiveFile x@(XObj _ i t) ctx args = return
  (evalError
    ctx
...
primitiveLine x@(XObj _ i t) ctx [] = case i of
  Just info ->
    return (ctx, Right (XObj (Num IntTy (fromIntegral (infoLine info))) i t))
  Nothing ->
    return (evalError ctx ("No information about object " ++ pretty x) (info x))
primitiveLine x@(XObj _ i t) ctx [XObj _ mi _] = case mi of
  Just info ->
    return (ctx, Right (XObj (Num IntTy (fromIntegral (infoLine info))) i t))
  Nothing ->
    return (evalError ctx ("No information about object " ++ pretty x) (info x))
primitiveLine x@(XObj _ i t) ctx args = return
  (evalError
    ctx
...
primitiveColumn x@(XObj _ i t) ctx [] = case i of
  Just info ->
    return (ctx, Right (XObj (Num IntTy (fromIntegral (infoColumn info))) i t))
  Nothing ->
    return (evalError ctx ("No information about object " ++ pretty x) (info x))
primitiveColumn x@(XObj _ i t) ctx [XObj _ mi _] = case mi of
  Just info ->
    return (ctx, Right (XObj (Num IntTy (fromIntegral (infoColumn info))) i t))
  Nothing ->
    return (evalError ctx ("No information about object " ++ pretty x) (info x))
primitiveColumn x@(XObj _ i t) ctx args = return
  (evalError
    ctx
      ...
      case def of
              ...
              case snd currentImplementations of
                ...
                Right old@(XObj (Lst impls) inf ty) ->
                  ...
                  in
                    return
                      ( ctx'
                        { contextGlobalEnv = envInsertAt
                                               global
                                               (getPath defobj)
                                               (Binder adjustedMeta defobj)
                        }
                      , dynamicNil
                      )
                _ ->
                  let
                    impls = XObj (Lst [x]) (Just dummyInfo) (Just DynamicTy)
                    adjustedMeta = meta
                      { getMeta = Map.insert "implements" impls (getMeta meta)
                      }
                  in
                    return
                      ( ctx'
        ...
        Nothing -> do
          ...
          primitiveMetaSet
            ...
            ctx
            [ i
            , XObj (Str "implements") (Just dummyInfo) (Just StringTy)
            , XObj (Lst [x])          (Just dummyInfo) (Just DynamicTy)
            ]
primitiveImplements xobj ctx [x, y] =
  return $ evalError ctx ("`implements` expects symbol arguments.") (info x)
primitiveImplements x@(XObj _ i t) ctx args = return $ evalError
  ctx
  ("`implements` expected 2 arguments, but got " ++ show (length args))
  (info x)

registerInInterfaceIfNeeded
  :: Context -> SymPath -> SymPath -> Ty -> Either String Context
registerInInterfaceIfNeeded ctx path@(SymPath _ _) interface@(SymPath [] name) definitionSignature
  = let typeEnv = getTypeEnv (contextTypeEnv ctx)
    in
      case lookupInEnv interface typeEnv of
          ...
          -> if checkKinds interfaceSignature definitionSignature
            then -- N.B. the xobjs aren't important here--we only care about types,
              ...
              if isRight $ solve
                ...
                then
                  let
                    updatedInterface = XObj
                      (Lst
                        [ XObj
                          (Interface interfaceSignature
                                     (addIfNotPresent path paths)
                          )
                          ii
                          it
                        , isym
                  ...
                  in
                    return $ ctx
                      { contextTypeEnv = TypeEnv
                        (extendEnv typeEnv name updatedInterface)
                      }
                else Left
                  (  "[INTERFACE ERROR] "
                  ++ show path
              ...
              ++ show path
              ++ ":"
              ++ " One or more types in the interface implementation "
              ++ show definitionSignature
        ...
        Just (_, Binder _ x) -> error
          (  "Can't implement the non-interface '"
          ++ name
...
registerDefnOrDefInInterfaceIfNeeded
  :: Context -> XObj -> SymPath -> Either String Context
registerDefnOrDefInInterfaceIfNeeded ctx xobj interface = case xobj of
...
define hidden ctx@(Context globalEnv _ typeEnv _ proj _ _ _) annXObj =
  let
    previousType = case lookupInEnv (getPath annXObj) globalEnv of
      Just (_, Binder _ found) -> ty found
      Nothing                  -> Nothing
    previousMeta = existingMeta globalEnv annXObj
    adjustedMeta = if hidden
      then previousMeta
        { getMeta = Map.insert "hidden" trueXObj (getMeta previousMeta)
        }
      else previousMeta
    fppl = projectFilePathPrintLength proj
  in
    case annXObj of
      XObj (Lst (XObj (Defalias _) _ _ : _)) _ _ -> return
        (ctx
          { contextTypeEnv = TypeEnv
                               (envInsertAt (getTypeEnv typeEnv)
                                            (getPath annXObj)
                                            (Binder adjustedMeta annXObj)
                               )
          }
        )
      XObj (Lst (XObj (Deftype _) _ _ : _)) _ _ -> return
        (ctx
          { contextTypeEnv = TypeEnv
                               (envInsertAt (getTypeEnv typeEnv)
                                            (getPath annXObj)
      ...
      XObj (Lst (XObj (DefSumtype _) _ _ : _)) _ _ -> return
        (ctx
          { contextTypeEnv = TypeEnv
                               (envInsertAt (getTypeEnv typeEnv)
                                            (getPath annXObj)
      ...
      _ -> do
        ...
        case previousType of
          Just previousTypeUnwrapped ->
            unless (areUnifiable (forceTy annXObj) previousTypeUnwrapped) $ do
              putStrWithColor
                Blue
                (  "[WARNING] Definition at "
                ++ prettyInfoFromXObj annXObj
                ++ " changed type of '"
                ++ show (getPath annXObj)
                ++ "' from "
        ...
        case Map.lookup "implements" (getMeta previousMeta) of
          Just (XObj (Lst interfaces) _ _) -> do
            let result = foldM
                  (\ctx (xobj, interface) ->
                    registerDefnOrDefInInterfaceIfNeeded ctx xobj interface
                  )
                  ctx
                  (zip (cycle [annXObj]) (map getPath interfaces))
            case result of
              Left err -> do
                case contextExecMode ctx of
                  Check ->
                    ...
                    in
                      putStrLn
                        (machineReadableInfoFromXObj fppl annXObj ++ " " ++ err)
                  _ -> putStrLnWithColor Red err
                return ctx
              Right ctx' -> return
                (ctx'
          ...
          _ -> return
            (ctx
              { contextGlobalEnv = envInsertAt globalEnv
                                               (getPath annXObj)
                                               (Binder adjustedMeta annXObj)
              }
            )
...
primitiveRegisterType _ ctx [x] = return
  (evalError ctx
             ("`register-type` takes a symbol, but it got " ++ pretty x)
             (info x)
  )
...
primitiveRegisterType _ ctx _ = return
  (evalError
    ctx
...
primitiveRegisterTypeWithoutFields
  :: Context -> String -> (Maybe String) -> IO (Context, Either EvalError XObj)
primitiveRegisterTypeWithoutFields ctx t override = do
  let pathStrings    = contextPath ctx
      ...
      typeDefinition = XObj
        (Lst
          [ XObj (ExternalType override) Nothing Nothing
          , XObj (Sym path Symbol)       Nothing Nothing
          ]
        )
        Nothing
  ...
  return
    ( ctx
      { contextTypeEnv = TypeEnv
                           (extendEnv (getTypeEnv typeEnv) t typeDefinition)
      }
    , dynamicNil
    )

primitiveRegisterTypeWithFields
  :: Context
  -> XObj
  -> String
  -> (Maybe String)
...
primitiveRegisterTypeWithFields ctx x t override members = do
  let pathStrings       = contextPath ctx
      ...
      preExistingModule = case lookupInEnv (SymPath pathStrings t) globalEnv of
        ...
        _ -> Nothing
  case
      bindingsForRegisteredType typeEnv
                                globalEnv
                                pathStrings
                                t
                                [members]
        ...
        let
          ...
          ctx' =
            (ctx
              { contextGlobalEnv = envInsertAt
                                     globalEnv
                                     (SymPath pathStrings typeModuleName)
                                     (Binder emptyMeta typeModuleXObj)
              , contextTypeEnv   = TypeEnv
                (extendEnv (getTypeEnv typeEnv) t typeDefinition)
              }
            )
...
notFound ctx x path = return
  (evalError ctx ("I can’t find the symbol `" ++ show path ++ "`") (info x))
...
primitiveInfo _ ctx [target@(XObj (Sym path@(SymPath _ name) _) _ _)] = do
 ...
 where
  printer env allowLookupInALL errNotFound binderPair = do
    ...
    case binderPair of
      Just (_, binder@(Binder metaData x@(XObj _ (Just i) _))) -> do
        liftIO $ putStrLn (show binder ++ "\nDefined at " ++ prettyInfo i)
        printDoc metaData proj x
      Just (_, binder@(Binder metaData x)) -> do
        liftIO $ print binder
        printDoc metaData proj x
      Nothing
        | allowLookupInALL -> case multiLookupALL name env of
          [] -> if errNotFound
            then notFound ctx target path
            else return (ctx, dynamicNil)
          binders -> do
            liftIO $ mapM_
              (\(env, binder@(Binder metaData x@(XObj _ i _))) -> case i of
                Just i' -> do
                  putStrLnWithColor
                    White
                    (show binder ++ "\nDefined at " ++ prettyInfo i')
  ...
  printDoc metaData proj x = do
    case Map.lookup "doc" (getMeta metaData) of
      ...
      Nothing                   -> return ()
    liftIO $ when (projectPrintTypedAST proj) $ putStrLnWithColor
      Yellow
      (prettyTyped x)
    return (ctx, dynamicNil)
primitiveInfo _ ctx [notName] = argumentErr ctx "info" "a name" "first" notName
...
dynamicOrMacroWith ctx producer ty name body = do
  ...
  return
    ( ctx { contextGlobalEnv = envInsertAt globalEnv path (Binder meta elem) }
    , dynamicNil
    )
...
primitiveType _ ctx [x@(XObj (Sym path@(SymPath [] name) _) _ _)] = do
  ...
  case lookupInEnv path env of
    ...
    Nothing          -> case multiLookupALL name env of
      []      -> notFound ctx x path
      binders -> liftIO $ do
        mapM_ (\(env, binder) -> putStrLnWithColor White (show binder)) binders
        return (ctx, dynamicNil)
...
primitiveType _ ctx [x] = return
  (evalError ctx ("Can't get the type of non-symbol: " ++ pretty x) (info x))
...
primitiveMembers _ ctx [target] = do
  ...
  case bottomedTarget env target of
    XObj (Sym path@(SymPath _ name) _) _ _ ->
      case lookupInEnv path (getTypeEnv typeEnv) of
        Just (_, Binder _ (XObj (Lst [XObj (Deftype structTy) Nothing Nothing, XObj (Sym (SymPath pathStrings typeName) Symbol) Nothing Nothing, XObj (Arr members) _ _]) _ _))
          -> return
            ( ctx
            , Right
              (XObj
                (Arr
                  (map (\(a, b) -> XObj (Lst [a, b]) Nothing Nothing)
                       (pairwise members)
                  )
                )
                Nothing
        ...
        Just (_, Binder _ (XObj (Lst (XObj (DefSumtype structTy) Nothing Nothing : XObj (Sym (SymPath pathStrings typeName) Symbol) Nothing Nothing : sumtypeCases)) _ _))
          -> return
            ( ctx
            , Right
              (XObj (Arr (concatMap getMembersFromCase sumtypeCases))
                    Nothing
                    Nothing
              )
            )
         where
          ...
          getMembersFromCase (XObj (Lst members) _ _) = map
            (\(a, b) -> XObj (Lst [a, b]) Nothing Nothing)
            (pairwise members)
        ...
        _ -> return
          (evalError
            ctx
            (  "Can't find a struct type named '"
            ++ name
            ++ "' in type environment"
            )
            (info target)
    ...
    _ -> return
      (evalError ctx
                 ("Can't get the members of non-symbol: " ++ pretty target)
                 (info target)
      )
 where
  bottomedTarget env target = case target of
    XObj (Sym targetPath _) _ _ -> case lookupInEnv targetPath env of
...
primitiveMetaSet _ ctx [target@(XObj (Sym path@(SymPath _ name) _) _ _), XObj (Str key) _ _, value]
      ...
      Nothing ->
        ...
        Nothing -> case path of
          ...
          (SymPath _ _) -> return
            (evalError
              ctx
              (  "`meta-set!` failed, I can't find the symbol `"
              ++ show path
              ++ "`"
              )
              (info target)
 ...
 where
  setMetaOn
    :: Context -> Maybe Env -> Binder -> IO (Context, Either EvalError XObj)
  setMetaOn ctx foundEnv binder@(Binder metaData xobj) = do
...
retroactivelyRegisterInterfaceFunctions ctx interface@(SymPath _ inter) = do
  let env       = contextGlobalEnv ctx
      ...
      resultCtx = foldl'
        (\maybeCtx binder -> case maybeCtx of
          Right ok -> registerDefnOrDefInInterfaceIfNeeded
            ok
            (binderXObj binder)
            interface
          Left err -> Left err
...
primitiveDefinterface xobj ctx [nameXObj@(XObj (Sym path@(SymPath [] name) _) _ _), ty]
  = do
    ...
    case xobjToTy ty of
      Just t -> case lookupInEnv path typeEnv of
          ...
          let interface = defineInterface name t [] (info nameXObj)
              typeEnv' =
                  TypeEnv
                    (envInsertAt typeEnv
                                 (SymPath [] name)
                                 (Binder emptyMeta interface)
                    )
          in  do
                newCtx <- retroactivelyRegisterInterfaceFunctions
                  (ctx { contextTypeEnv = typeEnv' })
                  path
                return (newCtx, dynamicNil)
      Nothing -> return
        (evalError
          ctx
          ("Invalid type for interface `" ++ name ++ "`: " ++ pretty ty)
          (info ty)
        )
primitiveDefinterface _ ctx [name, _] = do
  return
    (evalError
      ctx
      (  "`definterface` expects a name as first argument, but got `"
      ++ pretty name
      ++ "`"
      )
      (info name)
...
primitiveRegister _ ctx [name, _] = return
  (evalError
    ctx
    (  "`register` expects a name as first argument, but got `"
    ++ pretty name
    ++ "`"
    )
    (info name)
...
primitiveRegister _ ctx [XObj (Sym (SymPath _ name) _) _ _, _, override] =
  return
    (evalError
      ctx
      (  "`register` expects a string as third argument, but got `"
      ++ pretty override
      ++ "`"
      )
      (info override)
    )
primitiveRegister _ ctx [name, _, _] = return
  (evalError
    ctx
    (  "`register` expects a name as first argument, but got `"
    ++ pretty name
...
primitiveRegister x ctx _ = return
  (evalError
    ctx
...
primitiveDeftype xobj ctx (name : rest) = case rest of
  ...
  _ -> deftype name
 where
  deftype name@(XObj (Sym (SymPath _ ty) _) _ _) = deftype' name ty []
    ...
    let
      ...
      (preExistingModule, existingMeta) =
        case lookupInEnv (SymPath pathStrings typeName) env of
          Just (_, Binder existingMeta (XObj (Mod found) _ _)) ->
            (Just found, existingMeta)
    ...
    case (nameXObj, typeVariables) of
        ...
        case
          ...
          of
            Right (typeModuleName, typeModuleXObj, deps) ->
              let
                structTy = StructTy (ConcreteNameTy typeName) okTypeVariables
                typeDefinition =
                  ...
                  (Lst
                    ...
                    : XObj (Sym (SymPath pathStrings typeName) Symbol)
                           Nothing
                           Nothing
                    : rest
                    )
                ...
                ctx' =
                  (ctx
                    { contextGlobalEnv = envInsertAt
                                           env
                                           (SymPath pathStrings typeModuleName)
                                           (Binder existingMeta typeModuleXObj)
                    , contextTypeEnv   = TypeEnv
                      (extendEnv (getTypeEnv typeEnv) typeName typeDefinition)
                    }
                  )
              in
                do
                  ...
                  let ctxWithInterfaceRegistrations =
                        -- Since these functions are autogenerated, we treat them as a special case and automatically implement the interfaces.
                                                      foldM
                        (\context (path, sig, interface) ->
                          registerInInterfaceIfNeeded context path interface sig
                        )
                        ctxWithDeps
                        [ ( SymPath (pathStrings ++ [typeModuleName]) "str"
                          , FuncTy [RefTy structTy (VarTy "q")]
                                   StringTy
                                   StaticLifetimeTy
                          , (SymPath [] "str")
                          )
      ...
      (_, Nothing) -> return
        (makeEvalError
          ctx
          Nothing
          ("Invalid type variables for type definition: " ++ pretty nameXObj)
          (info nameXObj)
        )
...
primitiveUse xobj ctx [XObj (Sym path _) _ _] = do
  let
    ...
    e' =
      if path `elem` useThese then e else e { envUseModules = path : useThese }
  case lookupInEnv path e of
    Just (_, Binder _ _) -> return
      ( ctx { contextGlobalEnv = envReplaceEnvAt env pathStrings e' }
      , dynamicNil
      )
    Nothing -> case lookupInEnv path env of
      Just (_, Binder _ _) -> return
        ( ctx { contextGlobalEnv = envReplaceEnvAt env pathStrings e' }
        , dynamicNil
        )
      Nothing -> return
        (evalError ctx
                   ("Can't find a module named '" ++ show path ++ "'")
                   (info xobj)
        )
...
primitiveMeta (XObj _ i _) ctx [XObj (Sym path _) _ _, XObj (Str key) _ _] = do
 ...
 where
  lookup p e = case lookupInEnv p e of
    Just (_, Binder metaData _) -> case Map.lookup key (getMeta metaData) of
      Just foundValue -> return (ctx, Right foundValue)
      Nothing         -> return (ctx, dynamicNil)
    Nothing -> return
      (evalError ctx ("`meta` failed, I can’t find `" ++ show path ++ "`") i)
...
primitiveDefined _ ctx [arg] = argumentErr ctx "defined" "a symbol" "first" arg
...
primitiveDeftemplate _ ctx [XObj (Sym (SymPath [] name) _) pinfo _, ty, XObj (Str declTempl) _ _, XObj (Str defTempl) _ _]
  = do
    ...
    case xobjToTy ty of
            ...
            (_, b@(Binder _ (XObj (Lst (XObj (Deftemplate template) _ _ : _)) _ _)))
              -> if isTypeGeneric t
                then
                  ...
                  let
                    templateCreator   = getTemplateCreator template
                    (registration, _) = instantiateTemplate
                      p
                      t
                      (templateCreator typeEnv globalEnv)
                    meta = existingMeta globalEnv registration
                    env' = envInsertAt globalEnv p (Binder meta registration)
                  in
      ...
      Nothing -> return
        (evalError ctx
                   ("I do not understand the type form in " ++ pretty ty)
                   (info ty)
        )
