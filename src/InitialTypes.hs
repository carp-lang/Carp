...
initialTypes typeEnv rootEnv root = evalState (visit rootEnv root) 0
 where
  ...
  visit env xobj = case obj xobj of
    ...
    Break ->
      return (Right (xobj { ty = Just (FuncTy [] UnitTy StaticLifetimeTy) }))
  ...
  visitSymbol env xobj symPath = case symPath of
    -- Symbols with leading ? are 'holes'.
    SymPath _ name@('?' : _) ->
      return (Right (xobj { ty = Just (VarTy name) }))
    SymPath _ (':' : _) -> return (Left (LeadingColon xobj))
    _                   -> case lookupInEnv symPath env of
      Just (foundEnv, binder) -> case ty (binderXObj binder) of
  ...
  visitMultiSym
    :: Env -> XObj -> [SymPath] -> State Integer (Either TypeError XObj)
  visitMultiSym _ xobj@(XObj (MultiSym name _) _ _) _ = do
    freshTy <- genVarTy
    return (Right xobj { ty = Just freshTy })
  visitInterfaceSym :: Env -> XObj -> State Integer (Either TypeError XObj)
  visitInterfaceSym env xobj@(XObj (InterfaceSym name) _ _) = do
    freshTy <- case lookupInEnv (SymPath [] name) (getTypeEnv typeEnv) of
      Just (_, Binder _ (XObj (Lst [XObj (Interface interfaceSignature _) _ _, _]) _ _))
        -> renameVarTys interfaceSignature
      Just (_, Binder _ x) -> error
        (  "A non-interface named '"
        ++ name
        ++ "' was found in the type environment: "
        ++ show x
        )
      Nothing -> genVarTy
    return (Right xobj { ty = Just freshTy })
  visitArray :: Env -> XObj -> State Integer (Either TypeError XObj)
  visitArray env (XObj (Arr xobjs) i _) = do
    ...
    return $ do
      ...
      Right
        (XObj (Arr okVisited)
              i
              (Just (StructTy (ConcreteNameTy "Array") [arrayVarTy]))
        )
  visitArray _ _ =
    error "The function 'visitArray' only accepts XObj:s with arrays in them."
  visitStaticArray :: Env -> XObj -> State Integer (Either TypeError XObj)
  visitStaticArray env (XObj (StaticArr xobjs) i _) = do
    visited    <- mapM (visit env) xobjs
    arrayVarTy <- genVarTy
    lt         <- genVarTy
    return $ do
      ...
      Right
        (XObj
          ...
          i
          (Just
            (RefTy (StructTy (ConcreteNameTy "StaticArray") [arrayVarTy]) lt)
          )
        )
  visitStaticArray _ _ = error
    "The function 'visitStaticArray' only accepts XObj:s with arrays in them."
  visitDictionary :: Env -> XObj -> State Integer (Either TypeError XObj)
  visitDictionary env (XObj (Dict xobjs) i _) = do
    visited    <- mapM (visit env) xobjs
    arrayVarTy <- genVarTy
    return $ do
      okVisited <- sequence visited
      Right
        (XObj (Dict okVisited)
              i
              (Just (StructTy (ConcreteNameTy "Dictionary") [arrayVarTy]))
        )
  visitDictionary _ _ =
    error
      "The function 'visitArray' only accepts XObj:s with dictionaries in them."
  getTys env argList = do
  ...
  visitList env xobj@(XObj (Lst xobjs) i _) = case xobjs of
    -- Defn
    [defn@(XObj (Defn _) _ _), nameSymbol@(XObj (Sym (SymPath _ name) _) _ _), XObj (Arr argList) argsi argst, body]
      -> do
        ...
        return $ do
          ...
          return
            (XObj
              (Lst [defn, nameSymbol, XObj (Arr okArgs) argsi argst, okBody])
              i
              funcTy
            )
    [defn@(XObj (Defn _) _ _), XObj (Sym _ _) _ _, XObj (Arr _) _ _] ->
      return (Left (NoFormsInBody xobj))
    XObj defn@(Defn _) _ _ : _ -> return (Left (InvalidObj defn xobj))
    -- Fn
    [fn@(XObj (Fn _ _) _ _), XObj (Arr argList) argsi argst, body] -> do
      (argTypes, returnType, funcScopeEnv) <- getTys env argList
      lt <- genVarTy
      ...
      return $ do
        ...
        okArgs <- sequence visitedArgs
        let final =
              XObj (Lst [fn, XObj (Arr okArgs) argsi argst, okBody]) i funcTy
    ...
    -- Def
    [def@(XObj Def _ _), nameSymbol, expression] -> do
    ...
    -- Let binding
    [letExpr@(XObj Let _ _), XObj (Arr bindings) bindi bindt, body] -> do
      wholeExprType <- genVarTy
      letScopeEnv   <- extendEnvWithLetBindings env bindings
      case letScopeEnv of
        Right okLetScopeEnv -> do
    ...
    [XObj Let _ _, XObj (Arr _) _ _] -> return (Left (NoFormsInBody xobj))
    XObj Let _ _ : XObj (Arr _) _ _ : _ ->
      return (Left (TooManyFormsInBody xobj))
    XObj Let _ _ : _ -> return (Left (InvalidObj Let xobj))
    -- If
    [ifExpr@(XObj If _ _), expr, ifTrue, ifFalse] -> do
      ...
      return $ do
        ...
        return
          (XObj (Lst [ifExpr, okExpr, okTrue, okFalse]) i (Just returnType))
    XObj If _ _ : _ -> return (Left (InvalidObj If xobj))
    -- Match
    matchExpr@(XObj (Match _) _ _) : expr : cases -> do
      visitedExpr  <- visit env expr
      visitedCases <- sequence <$> mapM
        (\(lhs, rhs) -> do
      ...
      return $ do
        ...
        return
          (XObj (Lst ([matchExpr, okExpr] ++ okCasesConcatenated))
                i
                (Just returnType)
          )
    ...
    [XObj While _ _, _] -> return (Left (NoFormsInBody xobj))
    XObj While _ _ : _ -> return (Left (TooManyFormsInBody xobj))
    -- Do
    ...
    -- Set!
    [setExpr@(XObj SetBang _ _), variable, value] -> do
    ...
    [theExpr@(XObj The _ _), typeXObj, value] -> do
      ...
      return $ do
        ...
        case xobjToTy typeXObj of
          Just okType ->
            return (XObj (Lst [theExpr, typeXObj, okValue]) i (Just okType))
    ...
    XObj (Lst [deref@(XObj Deref _ _), func]) xi xt : args ->
      ...
      return $ do
        ...
        okArgs <- visitedArgs
        return
          (XObj (Lst (XObj (Lst [deref, okFunc]) xi (Just derefTy) : okArgs))
                i
                (Just t)
          )
  ...
  extendEnvWithLetBindings
    :: Env -> [XObj] -> State Integer (Either TypeError Env)
  extendEnvWithLetBindings env xobjs =
   ...
   where
    createBinderForLetPair
      :: Either TypeError Env
      -> (XObj, XObj)
      -> State Integer (Either TypeError Env)
    createBinderForLetPair envOrErr (sym, expr) = case envOrErr of
      Left  err  -> return (Left err)
      Right env' -> case obj sym of
        (Sym (SymPath _ name) _) -> do
  ...
  extendEnvWithParamList env xobjs = do
   ...
   where
    ...
    createBinderForParam xobj = case obj xobj of
      (Sym (SymPath _ name) _) -> do
        t <- genVarTy
  ...
  extendEnvWithCaseMatch env caseRoot = do
   ...
   where
    ...
    createBindersForCaseVariable xobj@(XObj (Sym (SymPath _ name) _) _ _) =
      createBinderInternal xobj name
    createBindersForCaseVariable xobj@(XObj (MultiSym name _) _ _) =
      createBinderInternal xobj name
    createBindersForCaseVariable xobj@(XObj (InterfaceSym name) _ _) =
      createBinderInternal xobj name
    createBindersForCaseVariable xobj@(XObj (Lst lst) _ _) = do
      binders <- mapM createBindersForCaseVariable lst
      return (concat binders)
    createBindersForCaseVariable xobj@(XObj Ref _ _) = return []
    createBindersForCaseVariable x =
      error
        (  "Can't create binder for non-symbol in 'case' variable match:"
        ++ show x
        ) -- TODO: Should use proper error mechanism
    createBinderInternal :: XObj -> String -> State Integer [(String, Binder)]
    createBinderInternal xobj name = if isVarName name
...
helpWithParens outer@(XObj (Lst [inner@(XObj (Sym (SymPath _ name) _) _ _)]) _ _)
  ...
  | -- Unwrap
    otherwise
  = outer -- Keep wrapped
helpWithParens xobj = wrapInParens xobj
