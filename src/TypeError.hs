...
instance Show TypeError where
  show (SymbolMissingType xobj env) =
    "I couldn’t find a type for the symbol '"
      ++ getName xobj
      ++ "' at "
  ...
  show (DefnMissingType xobj) =
    "I couldn’t find a type for the function definition '"
      ++ getName xobj
      ++ "' at "
      ++ prettyInfoFromXObj xobj
      ++ ".\n\nIt might be too general. You could try adding a type hint using `the`."
  show (DefMissingType xobj) =
    "I couldn’t find a type for the variable definition '"
      ++ getName xobj
      ++ "' at "
      ++ prettyInfoFromXObj xobj
      ++ ".\n\nIt might be too general. You could try adding a type hint using `the`."
  show (ExpressionMissingType xobj) =
    "I couldn’t find a type for the expression '"
      ++ pretty xobj
      ++ "' at "
      ++ prettyInfoFromXObj xobj
      ++ ".\n\nIt might be too general. You could try adding a type hint using `the`."
  show (SymbolNotDefined symPath@(SymPath p _) xobj env) =
    "I couldn’t find the symbol '"
      ++ show symPath
      ++ "' at "
      ++ prettyInfoFromXObj xobj
      ++ ".\n\n"
  ...
  show (InvalidObj If xobj) =
    "I didn’t understand the `if` statement at "
      ++ prettyInfoFromXObj xobj
  ...
  show (WrongArgCount xobj expected actual) =
    "You used the wrong number of arguments in '"
      ++ getName xobj
      ++ "' at "
      ++ prettyInfoFromXObj xobj
      ++ ". I expected "
  ...
  show (NotAFunction xobj) =
    "You are trying to call the non-function `"
      ++ getName xobj
      ++ "` at "
      ++ prettyInfoFromXObj xobj
      ++ "."
  ...
  show (UnificationFailed constraint@(Constraint a b aObj bObj ctx _) mappings constraints)
    = "I can’t match the types `"
      ++ show (recursiveLookupTy mappings a)
      ++ "` and `"
   ...
   where
      ...
      -- "All constraints:\n" ++ show constraints ++ "\n\n" ++
      -- "Mappings: \n" ++ show mappings ++ "\n\n"
    extra = if ctx == aObj || ctx == bObj
      then ""
      else " within `" ++ snip (pretty ctx) ++ "`"
  ...
  show (CantDisambiguateInterfaceLookup xobj name theType options) =
    "I found an ambiguous interface `"
      ++ name
      ++ "` of type `"
      ++ show theType
      ++ "` at "
  ...
  show (NoMatchingSignature xobj originalName theType options) =
    "I can’t find any implementation for the interface `"
      ++ originalName
      ++ "` of type "
      ++ show theType
      ++ " at "
  ...
  show (LetCantReturnRefTy xobj t) =
    "`let` expressions can’t return references. "
      ++ pretty xobj
      ++ " : "
      ++ show t
      ++ " at "
  ...
  show (GettingReferenceToUnownedValue xobj) =
    "You’re referencing a given-away value `"
      ++ pretty xobj
      ++ "` at "
  ...
  show (CannotConcretize xobj) =
    "I’m unable to concretize the expression '"
      ++ pretty xobj
      ++ "' at "
      ++ prettyInfoFromXObj xobj
      ++ ".\n\nIt might be too general. You could try adding a type hint using `the`."
  show (TooManyAnnotateCalls xobj) =
    "There were too many annotation calls when annotating `"
      ++ pretty xobj
      ++ "` at "
      ++ prettyInfoFromXObj xobj
  ...
  show (NotAmongRegisteredTypes t xobj) =
    "I can’t find a definition for the type `"
      ++ show t
      ++ "` at "
      ++ prettyInfoFromXObj xobj
      ++ ".\n\nWas it registered?"
  ...
  show (DuplicateBinding xobj) =
    "I encountered a duplicate binding `"
      ++ pretty xobj
      ++ "` inside the `let` at "
      ++ prettyInfoFromXObj xobj
      ++ "."
  show (DefinitionsMustBeAtToplevel xobj) =
    "I encountered a definition that was not at top level: `"
      ++ pretty xobj
      ++ "`"
  ...
  show (UninhabitedConstructor ty xobj got wanted) =
    "Can't use a struct or sumtype constructor without arguments as a member type at "
      ++ prettyInfoFromXObj xobj
...
machineReadableErrorStrings fppl err = case err of
  (UnificationFailed constraint@(Constraint a b aObj bObj _ _) mappings constraints)
    -> [ machineReadableInfoFromXObj fppl aObj
         ++ " Inferred "
         ++ showTypeFromXObj mappings aObj
         ++ ", can't unify with "
         ++ show (recursiveLookupTy mappings b)
         ++ "."
       , machineReadableInfoFromXObj fppl bObj
         ++ " Inferred "
         ++ showTypeFromXObj mappings bObj
         ++ ", can't unify with "
         ++ show (recursiveLookupTy mappings a)
  ...
  (SymbolNotDefined symPath xobj _) ->
    [ machineReadableInfoFromXObj fppl xobj
        ++ " Trying to refer to an undefined symbol '"
        ++ show symPath
        ++ "'."
    ]
  (SymbolMissingType xobj env) ->
  ...
  (WrongArgCount xobj expected actual) ->
    [ machineReadableInfoFromXObj fppl xobj
        ++ " Wrong argument count in call to '"
        ++ getName xobj
        ++ "' (expected "
        ++ show expected
        ++ ", received "
  ...
  (CantDisambiguateInterfaceLookup xobj name theType options) ->
    [ machineReadableInfoFromXObj fppl xobj
        ++ " Can't disambiguate interface lookup symbol '"
        ++ name
        ++ "' of type "
        ++ show theType
        ++ "\nPossibilities:\n    "
  ...
  (SeveralExactMatches xobj name theType options) ->
    [ machineReadableInfoFromXObj fppl xobj
        ++ " Several exact matches for interface lookup symbol '"
        ++ name
        ++ "' of type "
        ++ show theType
        ++ "\nPossibilities:\n    "
  ...
  (NoMatchingSignature xobj originalName theType options) ->
    [ machineReadableInfoFromXObj fppl xobj
        ++ " Can't find matching lookup for symbol '"
        ++ originalName
        ++ "' of type "
        ++ show theType
        ++ "\nNone of the possibilities have the correct signature:\n    "
  ...
  (LeadingColon xobj) ->
    [ machineReadableInfoFromXObj fppl xobj
        ++ " Symbol '"
        ++ pretty xobj
        ++ "' starting with a colon (reserved for REPL shortcuts)."
    ]
  -- (HolesFound holes) ->
  ...
  (GettingReferenceToUnownedValue xobj) ->
    [ machineReadableInfoFromXObj fppl xobj
        ++ " Referencing a given-away value '"
        ++ pretty xobj
        ++ "'."
    ]
  (UsingUnownedValue xobj) ->
    [ machineReadableInfoFromXObj fppl xobj
        ++ " Using a given-away value '"
        ++ pretty xobj
        ++ "'."
    ]
  (UsingCapturedValue xobj) ->
  ...
  (MainCanOnlyReturnUnitOrInt xobj t) ->
    [ machineReadableInfoFromXObj fppl xobj
        ++ " Main function can only return Int or (), got "
        ++ show t
        ++ "."
    ]
  (MainCannotHaveArguments xobj c) ->
    [ machineReadableInfoFromXObj fppl xobj
        ++ " Main function can not have arguments, got "
        ++ show c
        ++ "."
    ]
  (TooManyAnnotateCalls xobj) ->
    [ machineReadableInfoFromXObj fppl xobj
        ++ " Too many annotate calls (infinite loop) when annotating '"
        ++ pretty xobj
        ++ "'."
    ]
  ...
  (CannotSetVariableFromLambda variable xobj) ->
    [ machineReadableInfoFromXObj fppl variable
        ++ " Can't set! '"
        ++ pretty variable
        ++ "' from inside of a lambda."
    ]
  (CannotConcretize xobj) ->
    [ machineReadableInfoFromXObj fppl xobj
        ++ " Unable to concretize '"
        ++ pretty xobj
        ++ "'."
    ]
  (DoesNotMatchSignatureAnnotation xobj sigTy) ->
    [ machineReadableInfoFromXObj fppl xobj
        ++ "Definition does not match 'sig' annotation "
        ++ show sigTy
        ++ ", actual type is "
        ++ show (forceTy xobj)
    ]
  ...
  (InvalidMemberType t xobj) ->
    [ machineReadableInfoFromXObj fppl xobj
        ++ " Can't use '"
        ++ show t
        ++ "' as a type for a member variable."
    ]
  ...
  (UnevenMembers xobjs) ->
    [ machineReadableInfoFromXObj fppl (head xobjs)
        ++ " Uneven nr of members / types: "
        ++ joinWithComma (map pretty xobjs)
    ]
...
joinedMachineReadableErrorStrings fppl err =
...
recursiveLookupTy mappings t = case t of
  ...
  (RefTy r lt) ->
    RefTy (recursiveLookupTy mappings r) (recursiveLookupTy mappings lt)
  (PointerTy p) -> PointerTy (recursiveLookupTy mappings p)
  (StructTy n innerTys) ->
    StructTy n (map (recursiveLookupTy mappings) innerTys)
  (FuncTy argTys retTy ltTy) -> FuncTy
...
showTypeFromXObj mappings xobj = case ty xobj of
...
makeEvalError
  :: Context
  -> Maybe TypeError.TypeError
  -> String
  -> Maybe Info
  -> (Context, Either EvalError a)
