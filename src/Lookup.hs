...
lookupInEnv (SymPath [] name) env = case Map.lookup name (envBindings env) of
...
lookupInEnv path@(SymPath (p : ps) name) env =
  case Map.lookup p (envBindings env) of
    Just (Binder _ xobj) -> case xobj of
...
findAllGlobalVariables env = concatMap finder (envBindings env)
 where
  finder :: Binder -> [Binder]
  finder def@(Binder _ (XObj (Lst (XObj Def _ _ : _)) _ _)) = [def]
  finder (Binder _ (XObj (Mod innerEnv) _ _)) = findAllGlobalVariables innerEnv
  finder _ = []
...
multiLookupInternal allowLookupInAllModules name rootEnv = recursiveLookup
  rootEnv
 where
  ...
  importsNormal env = mapMaybe
    (\path -> fmap getEnvFromBinder (lookupInEnv path env))
    (envUseModules env)
  ...
  importsLookup env =
    let envs =
            (if allowLookupInAllModules then importsAll else importsNormal) env
...
lookupImplementations interface env =
  let binders = lookupByMeta "implements" env in filter isImpl binders
 where
  isImpl (Binder meta _) = case Map.lookup "implements" (getMeta meta) of
    Just (XObj (Lst interfaces) _ _) ->
      interface `elem` (map getPath interfaces)
    _ -> False

getEnvFromBinder :: (a, Binder) -> Env
getEnvFromBinder (_, Binder _ (XObj (Mod foundEnv) _ _)) = foundEnv
getEnvFromBinder (_, Binder _ err) =
  error ("Can't handle imports of non modules yet: " ++ show err)
    ...
    let fromParent = case envParent rootEnv of
        ...
        fromUsedModules =
            let usedModules = envUseModules rootEnv
                envs        = mapMaybe
                  (\path -> fmap getEnvFromBinder (lookupInEnv path rootEnv))
                  usedModules
...
envInsertAt env (SymPath [] name) binder = envAddBinding env name binder
envInsertAt env (SymPath (p : ps) name) xobj =
  case Map.lookup p (envBindings env) of
    Just (Binder existingMeta (XObj (Mod innerEnv) i t)) ->
      let newInnerEnv = Binder
            existingMeta
            (XObj (Mod (envInsertAt innerEnv (SymPath ps name) xobj)) i t)
...
envReplaceEnvAt env (p : ps) replacement =
  case Map.lookup p (envBindings env) of
    Just (Binder _ (XObj (Mod innerEnv) i t)) ->
      let newInnerEnv = Binder
            emptyMeta
            (XObj (Mod (envReplaceEnvAt innerEnv ps replacement)) i t)
      in  env { envBindings = Map.insert p newInnerEnv (envBindings env) }
...
envAddBinding env name binder =
  env { envBindings = Map.insert name binder (envBindings env) }
...
addListOfBindings env bindingsToAdd =
  foldl' (\e (n, b) -> envAddBinding e n b) env bindingsToAdd
...
envIsExternal env = case envMode env of
  ExternalEnv  -> True
  InternalEnv  -> False
  RecursionEnv -> True
...
isExternalType typeEnv (PointerTy p) = isExternalType typeEnv p
...
isManaged typeEnv (StructTy (ConcreteNameTy name) _) =
  (name == "Array")
    ...
    || (case lookupInEnv (SymPath [] name) (getTypeEnv typeEnv) of
         ...
         Just (_, Binder _ (XObj (Lst (XObj (DefSumtype _) _ _ : _)) _ _)) ->
           True
         Just (_, Binder _ (XObj wrong _ _)) ->
           error ("Invalid XObj in type env: " ++ show wrong)
...
keysInEnvEditDistance (SymPath [] name) env distance =
  let candidates = Map.filterWithKey
        (\k _ -> levenshteinDistance defaultEditCosts k name < distance)
        (envBindings env)
...
keysInEnvEditDistance path@(SymPath (p : ps) name) env distance =
  case Map.lookup p (envBindings env) of
    Just (Binder _ xobj) -> case xobj of
      (XObj (Mod modEnv) _ _) ->
        keysInEnvEditDistance (SymPath ps name) modEnv distance
      _ -> []
    Nothing -> case envParent env of
...
envReplaceBinding s@(SymPath [] name) binder env =
  case Map.lookup name (envBindings env) of
    Just _  -> envAddBinding env name binder
    Nothing -> case envParent env of
      Just parent ->
        env { envParent = Just (envReplaceBinding s binder parent) }
      Nothing -> env
envReplaceBinding (SymPath p name) xobj e =
  error "TODO: cannot replace qualified bindings"
...
existingMeta globalEnv xobj = case lookupInEnv (getPath xobj) globalEnv of
