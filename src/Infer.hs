...
annotate typeEnv globalEnv xobj rootSig = do
  initiated                 <- initialTypes typeEnv globalEnv xobj
  (annotated, dependencies) <- annotateUntilDone typeEnv
                                                 globalEnv
                                                 initiated
                                                 rootSig
                                                 []
...
annotateUntilDone
  ...
  -> Int
  -> Either TypeError (XObj, [XObj])
annotateUntilDone typeEnv globalEnv xobj rootSig deps limiter = if limiter <= 0
  then Left (TooManyAnnotateCalls xobj)
    ...
    if xobj == xobj' -- Is it the same?
      ...
      else annotateUntilDone typeEnv
                             globalEnv
                             xobj'
                             rootSig
                             newDeps
...
solveConstraintsAndConvertErrorIfNeeded
  :: [Constraint] -> Either TypeError TypeMappings
solveConstraintsAndConvertErrorIfNeeded constraints = case solve constraints of
  Left failure@(UnificationFailure _ _) -> Left
