...
memberInfo typeEnv memberTy =
  let refOrNotRefType = if isManaged typeEnv memberTy
        then RefTy memberTy (VarTy "w")
        else memberTy -- OBS! The VarTy "w" here is dubious
  in  ( refOrNotRefType
      , if isManaged typeEnv memberTy then "&" else ""
...
memberPrn typeEnv env (memberName, memberTy) =
  let (refOrNotRefType, maybeTakeAddress, strFuncType) =
          memberInfo typeEnv memberTy
  in  case nameOfPolymorphicFunction typeEnv env strFuncType "prn" of
        Just strFunctionPath -> unlines
          [ "  temp = "
          ++ pathToC strFunctionPath
          ++ "("
          ++ maybeTakeAddress
          ++ "p->"
          ++ memberName
        ...
        Nothing -> if isExternalType typeEnv memberTy
          then unlines
            [ "  temp = malloc(11);"
            ...
            "  // Failed to find str function for "
            ++ memberName
            ++ " : "
            ++ show memberTy
            ++ "\n"
...
memberPrnSize typeEnv env (memberName, memberTy) =
  let (refOrNotRefType, maybeTakeAddress, strFuncType) =
          memberInfo typeEnv memberTy
  in
    case nameOfPolymorphicFunction typeEnv env strFuncType "prn" of
      Just strFunctionPath -> unlines
        [ "  temp = "
        ++ pathToC strFunctionPath
        ++ "("
      ...
      Nothing -> if isExternalType typeEnv memberTy
        then unlines
          ["  size +=  11;", "  if(temp) { CARP_FREE(temp); temp = NULL; }"]
        else
          "  // Failed to find str function for "
          ++ memberName
          ++ " : "
