...
expandAll
  :: DynamicEvaluator -> Context -> XObj -> IO (Context, Either EvalError XObj)
expandAll eval ctx root = do
  (ctx, fullyExpanded) <- expandAllInternal root
  return (ctx, fmap setNewIdentifiers fullyExpanded)
 where
  expandAllInternal xobj = do
...
expand
  :: DynamicEvaluator -> Context -> XObj -> IO (Context, Either EvalError XObj)
expand eval ctx xobj = case obj xobj of
 ...
 where
  ...
  expandList (XObj (Lst xobjs) i t) = do
    ...
    case xobjs of
      ...
      [defnExpr@(XObj (Defn _) _ _), name, args, body] -> do
      ...
      [defExpr@(XObj Def _ _), name, expr] -> do
      ...
      [ifExpr@(XObj If _ _), condition, trueBranch, falseBranch] -> do
        ...
        return
          ...
          , do
            ...
            let wrappedTrue = case okTrueBranch of
                  XObj (Lst (XObj Do _ _ : _)) _ _ -> okTrueBranch -- Has a do-expression already
                  _ -> XObj (Lst [XObj Do Nothing Nothing, okTrueBranch])
                            (info okTrueBranch)
                            Nothing
                wrappedFalse = case okFalseBranch of
                  XObj (Lst (XObj Do _ _ : _)) _ _ -> okFalseBranch -- Has a do-expression already
                  _ -> XObj (Lst [XObj Do Nothing Nothing, okFalseBranch])
                            (info okFalseBranch)
                            Nothing

            Right
              (XObj (Lst [ifExpr, okCondition, wrappedTrue, wrappedFalse]) i t)
          )
      [letExpr@(XObj Let _ _), XObj (Arr bindings) bindi bindt, body] ->
        if even (length bindings)
          then do
            (ctx, bind) <- foldlM successiveExpand
                                  (ctx, Right [])
                                  (pairwise bindings)
              ...
              , do
                ...
                Right
                  (XObj
                    (Lst
                      [ letExpr
                      , XObj (Arr (concat okBindings)) bindi bindt
                      , okBody
                      ]
                    )
                    i
       ...
       where
        successiveExpand (ctx, acc) (n, x) = case acc of
      ...
      matchExpr@(XObj (Match _) _ _) : (expr : rest)
        | null rest -> return
          (evalError ctx "I encountered a `match` without forms" (info xobj))
        | even (length rest) -> do
          (ctx   , expandedExpr ) <- expand eval ctx expr
          (newCtx, expandedPairs) <- foldlM successiveExpand
                                            (ctx, Right [])
                                            (pairwise rest)
          return
            ( newCtx
            , do
              ...
              Right
                (XObj
                  (Lst (matchExpr : okExpandedExpr : (concat okExpandedPairs)))
                  i
                  t
                )
            )
       ...
       where
        successiveExpand (ctx, acc) (l, r) = case acc of
          Left  err -> return (ctx, acc)
          Right lst -> do
      ...
      doExpr@(XObj Do _ _) : expressions -> do
        (newCtx, expandedExpressions) <- foldlM successiveExpand
                                                (ctx, Right [])
                                                expressions
        return
          ( newCtx
      ...
      [withExpr@(XObj With _ _), _, _] -> return
        (evalError
          ctx
      ...
      XObj Mod{} _ _ : _ -> return
        (evalError ctx
                   ("I can’t evaluate the module `" ++ pretty xobj ++ "`")
                   (info xobj)
        )
      f : args -> do
        (ctx' , expandedF   ) <- expand eval ctx f
  ...
  expandArray (XObj (Arr xobjs) i t) = do
  ...
  expandSymbol (XObj (Sym path _) _ _) =
    case lookupInEnv path (contextEnv ctx) of
      Just (_, Binder _ (XObj (Lst (XObj (External _) _ _ : _)) _ _)) ->
        Right xobj
      Just (_, Binder _ (XObj (Lst (XObj (Instantiate _) _ _ : _)) _ _)) ->
        Right xobj
      Just (_, Binder _ (XObj (Lst (XObj (Deftemplate _) _ _ : _)) _ _)) ->
        Right xobj
      Just (_, Binder _ (XObj (Lst (XObj (Defn _) _ _ : _)) _ _)) -> Right xobj
      Just (_, Binder _ (XObj (Lst (XObj Def _ _ : _)) _ _)     ) -> Right xobj
      Just (_, Binder _ (XObj (Lst (XObj (Defalias _) _ _ : _)) _ _)) ->
        Right xobj
  ...
  successiveExpand (ctx, acc) e = case acc of
    Left  err -> return (ctx, acc)
    Right lst -> do
...
setNewIdentifiers root = let final = evalState (visit root) 0 in final
 where
  ...
  visitList (XObj (Lst xobjs) i t) = do
  ...
  visitArray (XObj (Arr xobjs) i t) = do
    visited <- mapM visit xobjs
  ...
  visitStaticArray (XObj (StaticArr xobjs) i t) = do
    visited <- mapM visit xobjs
...
replaceSourceInfo newFile newLine newColumn root = visit root
 where
  ...
  setNewInfo xobj = case info xobj of
    Just i ->
      (xobj
        { info = Just
          (i { infoFile = newFile, infoLine = newLine, infoColumn = newColumn })
        }
      )
    Nothing -> xobj

replaceSourceInfoOnXObj :: Maybe Info -> XObj -> XObj
replaceSourceInfoOnXObj newInfo xobj = case newInfo of
