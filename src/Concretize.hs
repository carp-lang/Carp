...
concretizeXObj allowAmbiguityRoot typeEnv rootEnv visitedDefinitions root =
 ...
 where
  ...
  visit allowAmbig level env xobj@(XObj (Sym _ _) _ _) =
    visitSymbol allowAmbig env xobj
  visit allowAmbig level env xobj@(XObj (MultiSym _ _) _ _) =
    visitMultiSym allowAmbig env xobj
  visit allowAmbig level env xobj@(XObj (InterfaceSym _) _ _) =
    visitInterfaceSym allowAmbig env xobj
  visit allowAmbig level env xobj@(XObj (Lst _) i t) = do
    visited <- visitList allowAmbig level env xobj
    return $ do
      okVisited <- visited
      Right (XObj (Lst okVisited) i t)
  visit allowAmbig level env xobj@(XObj (Arr arr) i (Just t)) = do
  ...
  visit allowAmbig level env xobj@(XObj (StaticArr arr) i (Just t)) = do
    visited <- fmap sequence (mapM (visit allowAmbig level env) arr)
    concretizeTypeOfXObj typeEnv xobj
    return $ do
      okVisited <- visited
  ...
  visitList
    :: Bool -> Level -> Env -> XObj -> State [XObj] (Either TypeError [XObj])
  ...
  visitList _ Toplevel env (XObj (Lst [defn@(XObj (Defn _) _ _), nameSymbol, args@(XObj (Arr argsArr) _ _), body]) _ t)
    = do
      ...
      concretizeTypeOfXObj typeEnv body
      visitedBody <- visit allowAmbig
                           Inside
                           (incrementEnvNestLevel envWithArgs)
                           body
      return $ do
        okBody <- visitedBody
  ...
  visitList allowAmbig _ env (XObj (Lst [XObj (Fn _ _) fni fnt, args@(XObj (Arr argsArr) ai at), body]) i t)
    ...
    -- the C code.
      ...
      let
        ...
        functionEnv = Env Map.empty
                          (Just env)
                          Nothing
                          []
                          InternalEnv
                          (envFunctionNestingLevel env)
        envWithArgs = foldl'
          (\e arg@(XObj (Sym (SymPath _ argSymName) _) _ _) ->
            extendEnv e argSymName arg
          )
      ...
      case visitedBody of
        Right okBody ->
          let -- Analyse the body of the lambda to find what variables it captures
            ...
            capturedVars    = filter
              (\xobj -> obj (toGeneralSymbol xobj) `notElem` argObjs)
              capturedVarsRaw
            ...
            lambdaPath        = SymPath
              path
              (  "_Lambda_"
              ++ lambdaToCName name (envFunctionNestingLevel envWithArgs)
              ++ "_"
              ++ show (infoIdentifier ii)
              )
            lambdaNameSymbol =
              XObj (Sym lambdaPath Symbol) (Just dummyInfo) Nothing
              ...
              then args
                ...
                (Arr
                  ( XObj
                      ...
                      (Just
                        (PointerTy
                          (StructTy (ConcreteNameTy environmentTypeName) [])
                        )
                      )
                  : argsArr
                  )
            ...
            lambdaCallback = XObj
              (Lst
                [ XObj (Defn (Just (Set.fromList capturedVars)))
                       (Just dummyInfo)
                       Nothing
                , lambdaNameSymbol
                , extendedArgs
            ...
            environmentStructTy =
              StructTy (ConcreteNameTy environmentTypeName) []
            environmentStruct = XObj
              (Lst
                [ XObj (Deftype environmentStructTy) Nothing Nothing
                , XObj (Sym (SymPath [] environmentTypeName) Symbol)
                       Nothing
                       Nothing
            ...
            deleteFnTy =
              typesDeleterFunctionType (PointerTy environmentStructTy)
            deleteFnTemplate        = concreteDeleteTakePtr typeEnv env pairs
            (deleteFn, deleterDeps) = instantiateTemplate
              (SymPath [] (environmentTypeName ++ "_delete"))
              deleteFnTy
              deleteFnTemplate
            copyFnTy           = typesCopyFunctionType environmentStructTy
            copyFnTemplate     = concreteCopyPtr typeEnv env pairs
            (copyFn, copyDeps) = instantiateTemplate
              (SymPath [] (environmentTypeName ++ "_copy"))
              copyFnTy
              copyFnTemplate
            -- The type env has to contain the lambdas environment struct for 'concretizeDefinition' to work:
            extendedTypeEnv =
              TypeEnv
                (extendEnv (getTypeEnv typeEnv)
                           environmentTypeName
                           environmentStruct
                )
          in
            case
              concretizeDefinition allowAmbig
                                   extendedTypeEnv
                                   env
                                   visitedDefinitions
                                   lambdaCallback
            ...
            of
              ...
              Right (concreteLiftedLambda, deps) -> do
                unless (any (isTypeGeneric . snd) pairs) $ do
                  modify (concreteLiftedLambda :)
                  modify (deps ++)
                  unless (null capturedVars) $ do
                ...
                return
                  (Right
                    [ XObj (Fn (Just lambdaPath) (Set.fromList capturedVars))
                           fni
                           fnt
                    , args
                    , okBody
  ...
  visitList allowAmbig level env (XObj (Lst [letExpr@(XObj Let _ _), XObj (Arr bindings) bindi bindt, body]) _ _)
    = do
      visitedBindings <- fmap sequence
                              (mapM (visit allowAmbig level env) bindings)
      ...
      return $ do
        ...
        return
          [letExpr, XObj (Arr okVisitedBindings) bindi bindt, okVisitedBody]
  ...
  visitList allowAmbig level env (XObj (Lst (func : args)) _ _) = do
    concretizeTypeOfXObj typeEnv func
  ...
  visitMatchCase
    ...
    -> Env
    -> (XObj, XObj)
    -> State [XObj] (Either TypeError [XObj])
  visitMatchCase allowAmbig level env (lhs, rhs) = do
    ...
    case lookupInEnv path env of
      Just (foundEnv, binder)
        ...
        -> let theXObj       = binderXObj binder
               ...
               typeOfVisited = fromMaybe
                 (error
                   (  "Missing type on "
                   ++ show xobj
                   ++ " at "
                   ++ prettyInfoFromXObj xobj
                   ++ " when looking up path "
                   ++ show path
                   )
                 )
                 t
           in  if --(trace $ "CHECKING " ++ getName xobj ++ " : " ++ show theType ++ " with visited type " ++ show typeOfVisited ++ " and visited definitions: " ++ show visitedDefinitions) $
                 ...
                 then
                   ...
                   of
                     Left  err              -> return (Left err)
                     Right (concrete, deps) -> do
                       modify (concrete :)
                       modify (deps ++)
                       return
                         (Right (XObj (Sym (getPath concrete) lookupMode) i t))
      ...
      in
        case filter (matchingSignature3 actualType) tysPathsModes of
          ...
          [(theType, singlePath, mode)] ->
            ...
            in
              case
                solve [Constraint theType t' fake1 fake2 fake1 OrdMultiSym]
              of
                Right mappings ->
                  let
                    replaced = replaceTyVars mappings t'
                    suffixed =
                      suffixTyVars ("_x" ++ show (infoIdentifier i')) replaced -- Make sure it gets unique type variables. TODO: Is there a better way?
                    normalSymbol = XObj (Sym singlePath mode) i (Just suffixed)
                  in
                    visitSymbol allowAmbig
                                ...
                                normalSymbol
                Left failure@(UnificationFailure _ _) -> return $ Left
                  (UnificationFailed (unificationFailure failure)
                                     (unificationMappings failure)
                                     []
                  )
                Left (Holes holes) -> return $ Left (HolesFound holes)
          severalPaths -> return (Right xobj)
  visitMultiSym _ _ _ = error "Not a multi symbol."
  visitInterfaceSym
    :: Bool -> Env -> XObj -> State [XObj] (Either TypeError XObj)
    ...
    case lookupInEnv (SymPath [] name) (getTypeEnv typeEnv) of
      Just (_, Binder _ (XObj (Lst [XObj (Interface interfaceSignature interfacePaths) _ _, _]) _ _))
        -> let
           ...
           in
             case filter (matchingSignature actualType) tysToPathsDict of
               [] -> return $ --(trace ("No matching signatures for interface lookup of " ++ name ++ " of type " ++ show actualType ++ " " ++ prettyInfoFromXObj xobj ++ ", options are:\n" ++ joinLines (map show tysToPathsDict))) $
                 ...
                 then Right xobj -- No exact match of types
                   ...
                   (NoMatchingSignature xobj name actualType tysToPathsDict)
               [(theType, singlePath)] ->
                 ...
                 let Just tt = t
                 in  if isTypeGeneric tt
                       then return (Right xobj)
                       else replace theType singlePath
               severalPaths ->
                 ...
                 case
                     filter (\(tt, _) -> typeEqIgnoreLifetimes actualType tt)
                            severalPaths
                   of
                     [] ->
       ...
       where
        replace theType singlePath =
          let normalSymbol =
                  XObj (Sym singlePath (LookupGlobal CarpLand AFunction)) i t -- TODO: Is it surely AFunction here? Could be AVariable as well...!?
          in  visitSymbol allowAmbig
                          env --  $ trace ("Replacing symbol " ++ pretty xobj ++ " with type " ++ show theType ++ " to single path " ++ show singlePath)
                          normalSymbol
      Nothing -> error ("No interface named '" ++ name ++ "' found.")

toGeneralSymbol :: XObj -> XObj
toGeneralSymbol (XObj (Sym path _) _ t) =
  XObj (Sym path Symbol) (Just dummyInfo) t
toGeneralSymbol x =
  error ("Can't convert this to a general symbol: " ++ show x)

-- | Find all lookups in a lambda body that should be captured by its environment
collectCapturedVars :: XObj -> [XObj]
collectCapturedVars root = removeDuplicates
  (map decreaseCaptureLevel (visit root))
 where
  removeDuplicates :: Ord a => [a] -> [a]
  removeDuplicates = Set.toList . Set.fromList
  decreaseCaptureLevel :: XObj -> XObj
  decreaseCaptureLevel (XObj (Sym path lookup) _ ty) = XObj
    (Sym
      path
      (case lookup of
        Symbol                -> Symbol
        ...
        LookupLocal (Capture n) ->
          if n <= 1 then Symbol else LookupLocal (Capture (n - 1))
      )
    )
    (Just dummyInfo)
    ty
  visit xobj = case obj xobj of
    ...
    _ -> []
  visitList :: XObj -> [XObj]
  visitList (XObj (Lst xobjs) _ _) = concatMap visit xobjs
  visitList _ =
    error "The function 'visitList' only accepts XObjs with lists in them."
  visitArray :: XObj -> [XObj]
  visitArray (XObj (Arr xobjs) _ _) = concatMap visit xobjs
  visitArray _ =
    error "The function 'visitArray' only accepts XObjs with arrays in them."
...
concretizeType _ ft@FuncTy{} =
  if isTypeGeneric ft then Right [] else Right [defineFunctionTypeAlias ft]
  ...
  case lookupInEnv (SymPath [] name) (getTypeEnv typeEnv) of
    Just (_, Binder _ (XObj (Lst (XObj (Deftype originalStructTy) _ _ : _ : rest)) _ _))
      -> if isTypeGeneric originalStructTy
        then instantiateGenericStructType typeEnv
                                          originalStructTy
                                          genericStructTy
                                          rest
        else Right []
    Just (_, Binder _ (XObj (Lst (XObj (DefSumtype originalStructTy) _ _ : _ : rest)) _ _))
      -> if isTypeGeneric originalStructTy
        then instantiateGenericSumtype typeEnv
                                       originalStructTy
                                       genericStructTy
                                       rest
        else Right []
    Just (_, Binder _ (XObj (Lst (XObj (ExternalType _) _ _ : _)) _ _)) ->
      Right []
    Just (_, Binder _ x) -> error ("Non-deftype found in type env: " ++ show x)
    Nothing              -> Right []
concretizeType env (RefTy rt _  ) = concretizeType env rt
concretizeType env (PointerTy pt) = concretizeType env pt
concretizeType _   t              = Right [] -- ignore all other types

-- | Given an generic struct type and a concrete version of it, generate all dependencies needed to use the concrete one.
-- TODO: Handle polymorphic constructors (a b).
instantiateGenericStructType
  :: TypeEnv -> Ty -> Ty -> [XObj] -> Either TypeError [XObj]
  ...
  -- Turn (deftype (A a) [x a, y a]) into (deftype (A Int) [x Int, y Int])
    ...
    in
      ...
      of
        Left e -> error (show e)
        Right mappings ->
          let concretelyTypedMembers =
                  replaceGenericTypeSymbolsOnMembers mappings memberXObjs
          in
            case
              validateMembers typeEnv originalTyVars concretelyTypedMembers
            of
              Left err -> Left err
              Right () ->
                let
                  deps = mapM (depsForStructMemberPair typeEnv)
                      ...
                      Right
                        $ XObj
                            (Lst
                              ...
                              : XObj
                                  (Sym (SymPath [] (tyToC genericStructTy))
                                       Symbol
                                  )
                                  Nothing
                                  Nothing
                              : [ XObj (Arr concretelyTypedMembers)
                                       Nothing
                                       Nothing
                                ]
                              )
...
depsForStructMemberPair typeEnv (_, tyXObj) = case xobjToTy tyXObj of
          ...
          of
            ...
            Right mappings ->
              ...
              in  case toCases typeEnv originalTyVars concretelyTypedCases of -- Don't care about the cases, this is done just for validation.
                    Left  err -> Left err
                    Right _   -> case deps of
                      Right okDeps ->
                        Right
                          $ XObj
                              (Lst
                                ( XObj (DefSumtype genericStructTy)
                                       Nothing
                                       Nothing
                                : XObj
                                    (Sym
                                      (SymPath [] (tyToC genericStructTy))
                                      Symbol
                                    )
                                    Nothing
                                    Nothing
  ...
  concat
    <$> mapM
          (\m -> case xobjToTy m of
            Just okTy -> concretizeType typeEnv okTy
            Nothing   -> error
              ("Failed to convert " ++ pretty m ++ " to a type: " ++ pretty x)
          )
          members
...
replaceGenericTypeSymbolsOnCase mappings singleCase@(XObj (Lst (caseName : caseMembers)) i t)
  ...
  where replacer memberXObj = replaceGenericTypeSymbols mappings memberXObj
...
modeFromPath env p = case lookupInEnv p env of
  ...
  Just (e, Binder _ found) -> case envMode e of
    ExternalEnv  -> LookupGlobal CarpLand (definitionMode found)
...
concretizeDefinition allowAmbiguity typeEnv globalEnv visitedDefinitions definition concreteType
  = let
      SymPath pathStrings name = getPath definition
      ...
      case definition of
        XObj (Lst (XObj (Defn _) _ _ : _)) _ _ ->
          ...
          in  case assignTypes mappings withNewPath of
                Right typed -> if newPath `elem` visitedDefinitions
                  then
                    return
                      (trace ("Already visited " ++ show newPath)
                             (withNewPath, [])
                      )
                  else do
                    (concrete, deps) <- concretizeXObj
                      allowAmbiguity
                      typeEnv
                      globalEnv
                      (newPath : visitedDefinitions)
                      typed
                    (managed, memDeps) <- manageMemory typeEnv
                                                       globalEnv
                                                       concrete
        ...
        XObj (Lst [XObj (External _) _ _, _]) _ _ -> if name == "NULL"
        ...
        err -> Left $ CannotConcretize definition

-- | Find ALL functions with a certain name, matching a type signature.
allFunctionsWithNameAndSignature env functionName functionType = filter
  (predicate . ty . binderXObj . snd)
  (multiLookupALL functionName env)
 where
  predicate (Just t) =
...
depsOfPolymorphicFunction
  :: TypeEnv -> Env -> [SymPath] -> String -> Ty -> [XObj]
...
depsForDeleteFunc typeEnv env t = if isManaged typeEnv t
  then depsOfPolymorphicFunction typeEnv
                                 env
                                 []
                                 "delete"
                                 (FuncTy [t] UnitTy StaticLifetimeTy)
  else []

-- | Helper for finding the 'copy' function for a type.
depsForCopyFunc :: TypeEnv -> Env -> Ty -> [XObj]
depsForCopyFunc typeEnv env t = if isManaged typeEnv t
  then depsOfPolymorphicFunction
    typeEnv
    env
    []
    "copy"
    (FuncTy [RefTy t (VarTy "q")] t StaticLifetimeTy)
  else []

-- | Helper for finding the 'str' function for a type.
depsForPrnFunc :: TypeEnv -> Env -> Ty -> [XObj]
depsForPrnFunc typeEnv env t = if isManaged typeEnv t
  then depsOfPolymorphicFunction
    typeEnv
    env
    []
    "prn"
    (FuncTy [RefTy t (VarTy "q")] StringTy StaticLifetimeTy)
  else depsOfPolymorphicFunction typeEnv
                                 env
                                 []
                                 "prn"
                                 (FuncTy [t] StringTy StaticLifetimeTy)

-- | The type of a type's str function.
typesStrFunctionType :: TypeEnv -> Ty -> Ty
typesStrFunctionType typeEnv memberType = if isManaged typeEnv memberType
...
prettyLifetimeMappings mappings = joinLines
...
manageMemory typeEnv globalEnv root =
  let (finalObj, finalState) =
          runState (visit root) (MemState (Set.fromList []) [] Map.empty)
  ...
  in    -- (trace ("Delete these: " ++ joinWithComma (map show (Set.toList deleteThese)))) $
      case finalObj of
        Left err -> Left err
        Right ok ->
          let newInfo = fmap (\i -> i { infoDelete = deleteThese }) (info ok)
          in    -- This final check of lifetimes works on the lifetimes mappings after analyzing the function form, and
              ...
              case
                  evalState
                    ...
                    (MemState (Set.fromList [])
                              []
                              (memStateLifetimes finalState)
                    )
                of
                  Left  err -> Left err
 ...
 where
  ...
  visitArray xobj@(XObj (Arr arr) _ _) = do
    mapM_ visit arr
  ...
  visitStaticArray xobj@(XObj (StaticArr arr) _ _) = do
    mapM_ visit arr
    results <- mapM unmanage arr
    case sequence results of
      Left  e -> return (Left e)
      Right _ ->
        ...
        let var = varOfXObj xobj
            Just (RefTy t@(StructTy (ConcreteNameTy "StaticArray") [_]) _) =
              ty xobj
            deleter =
              case
                  nameOfPolymorphicFunction
                    ...
                    "delete"
                of
                  Just pathOfDeleteFunc -> ProperDeleter pathOfDeleteFunc var
                  Nothing ->
  ...
  visitList xobj@(XObj (Lst lst) i t) = case lst of
         ...
         in    --case defnReturnType of
             ...
             do
               ...
               mapM_
                 (\cap -> modify
                   (\memState -> memState
                     { memStateDeleters = Set.insert
                                            (FakeDeleter cap)
                                            (memStateDeleters memState)
                     }
                   )
                 )
    ...
    -- Def
    [def@(XObj Def _ _), nameSymbol@(XObj (Sym _ _) _ _), expr] -> do
        ...
        do
          ...
          case result of
            ...
            Right _ -> do
              ...
              return $ do
                okBody     <- visitedBody
                okBindings <- fmap (concatMap (\(n, x) -> [n, x]))
                                   (sequence visitedBindings)
                return
                  (XObj
                    (Lst [letExpr, XObj (Arr okBindings) bindi bindt, okBody])
                    newInfo
                    t
                  )
    ...
    [setbangExpr@(XObj SetBang _ _), variable, value] ->
      let
        ...
        correctVariableAndMode = case variable of
      ...
      in
        case correctVariableAndMode of
          Left  err                         -> return (Left err)
          Right (okCorrectVariable, okMode) -> do
            MemState preDeleters _ _ <- get
            ...
            let
              deleters = case createDeleter okCorrectVariable of
                Just d  -> Set.fromList [d]
                Nothing -> Set.empty
              newVariable = case okMode of
    ...
    [addressExpr@(XObj Address _ _), value] -> do
      visitedValue <- visit value
      return $ do
        okValue <- visitedValue
        return (XObj (Lst [addressExpr, okValue]) i t)
    [theExpr@(XObj The _ _), typeXObj, value] -> do
      visitedValue <- visit value
    ...
    [refExpr@(XObj Ref _ _), value] -> do
      visitedValue <- visit value
      case visitedValue of
        Left  e            -> return (Left e)
        Right visitedValue -> do
    ...
    (XObj Deref _ _ : _) ->
      error
        "Shouldn't end up here, deref only works when calling a function, i.e. ((deref f) 1 2 3)."
    doExpr@(XObj Do _ _) : expressions -> do
    ...
    [whileExpr@(XObj While _ _), expr, body] -> do
      MemState preDeleters _ _       <- get
      visitedExpr                    <- visit expr
      ...
      return $ do
        ...
        let
          ...
          newExprInfo =
            setDeletersOnInfo objInfo (afterExprDeleters \\ preDeleters)
          newExpr = XObj objExpr newExprInfo objTy
        return (XObj (Lst [whileExpr, newExpr, okBody]) newInfo t)
    [ifExpr@(XObj If _ _), expr, ifTrue, ifFalse] -> do
      visitedExpr                         <- visit expr
      MemState preDeleters deps lifetimes <- get

      let (visitedTrue, stillAliveTrue) = runState
            (do
              v      <- visit ifTrue
          ...
          (visitedFalse, stillAliveFalse) = runState
            (do
              v      <- visit ifFalse
      ...
      let
        ...
        delsTrue =
          Set.union (deletedInFalse \\ deletedInBoth) createdAndDeletedInTrue
        delsFalse =
          Set.union (deletedInTrue \\ deletedInBoth) createdAndDeletedInFalse
        stillAliveAfter = preDeleters \\ Set.union deletedInTrue deletedInFalse
        depsAfter =
          memStateDeps stillAliveTrue ++ memStateDeps stillAliveFalse ++ deps -- Note: This merges all previous deps and the new ones, could be optimized..?!
        traceDeps = trace
          (  "IF-deleters for "
          ++ pretty xobj
          ++ " at "
          ...
          ++ "\n"
          ++ "stillAliveTrue: "
          ++ show (memStateDeleters stillAliveTrue)
          ++ "\n"
          ++ "stillAliveFalse: "
          ++ show (memStateDeleters stillAliveFalse)
          ++ "\n"
          ++ "createdInTrue: "
          ++ show createdInTrue
          ++ "\n"
          ...
          ++ show createdInFalse
          ++ "\n"
          ++ "createdAndDeletedInTrue: "
          ++ show createdAndDeletedInTrue
          ++ "\n"
          ++ "createdAndDeletedInFalse: "
          ++ show createdAndDeletedInFalse
          ++ "\n"
          ++ "deletedInTrue: "
          ++ show deletedInTrue
          ++ "\n"
      ...
      return $ do
        ...
        return
          (XObj
            (Lst [ifExpr, okExpr, del okTrue delsTrue, del okFalse delsFalse])
            i
            t
          )
    matchExpr@(XObj (Match _) _ _) : expr : cases ->
      ...
      case visitedExpr of
        Left  e             -> return (Left e)
        Right okVisitedExpr -> do
          unmanage okVisitedExpr
          MemState preDeleters deps lifetimes <- get
          ...
          case sequence vistedCasesAndDeps of
            ...
            Right okCasesAndDeps ->
              let visitedCases  = map fst okCasesAndDeps
                  ...
                  (finalXObj, postDeleters) =
                      figureOutStuff okVisitedExpr visitedCases preDeleters
              in  do
                    put
                      (MemState postDeleters (deps ++ depsFromCases) lifetimes)
                    manage xobj
     ...
     where
      ...
      figureOutStuff okVisitedExpr visitedCasesWithDeleters preDeleters =
        let
          postDeleters             = map fst visitedCasesWithDeleters
          ...
          deletersAfterTheMatch =
            Set.intersection preDeleters postDeletersIntersection
          ...
          postDeletersUnionPreExisting =
            Set.intersection postDeletersUnion preDeleters
          ...
          okVisitedCasesWithAllDeleters = zipWith
        ...
        in -- trace ("post deleters: " ++ show postDeleters)
          ...
          ( XObj
            (Lst
              (  [matchExpr, okVisitedExpr]
              ++ concat okVisitedCasesWithAllDeleters
              )
            )
            i
            t
    ...
    XObj (Lst [deref@(XObj Deref _ _), f]) xi xt : args -> do
  ...
  visitMatchCase
    ...
    -> State
         MemState
         (Either TypeError ((Set.Set Deleter, (XObj, XObj)), [XObj]))
  visitMatchCase (lhs@(XObj _ lhsInfo _), rhs@XObj{}) = do
    MemState preDeleters preDeps preLifetimes <- get
    _          <- visitCaseLhs lhs
  ...
  visitCaseLhs (XObj (Lst vars) _ _) = do
  ...
  visitCaseLhs (XObj Ref _ _) = return (Right [])
  visitCaseLhs x              = error ("Unhandled: " ++ show x)
  addToLifetimesMappingsIfRef :: Bool -> XObj -> State MemState ()
  addToLifetimesMappingsIfRef internal xobj = case ty xobj of
    Just (RefTy _ (VarTy lt)) -> do
    ...
    _ ->
      ...
      return ()
   where
    makeLifetimeMode xobj = if internal
      then LifetimeInsideFunction $ case xobj of
  ...
  checkThatRefTargetIsAlive xobj =
    -- TODO: Replace this whole thing with a function that collects all lifetime variables in a type.
                                   case ty xobj of
    Just (RefTy _ (VarTy lt)                      ) -> performCheck lt
    Just (FuncTy _ _                    (VarTy lt)) -> performCheck lt
    -- HACK (not exhaustive):
    Just (FuncTy _ (RefTy _ (VarTy lt)) _         ) -> performCheck lt
    _ ->
    ...
    performCheck lt = do
      ...
      case Map.lookup lt lifetimeMappings of
        Just (LifetimeInsideFunction deleterName) ->
          let matchingDeleters = Set.toList $ Set.filter
                (\case
  ...
  visitLetBinding
    :: (XObj, XObj) -> State MemState (Either TypeError (XObj, XObj))
  visitLetBinding (name, expr) = do
    visitedExpr <- visit expr
  ...
  visitArg xobj@(XObj _ _ (Just t)) = do
    ...
    case afterVisit of
      ...
      Left err -> return (Left err)
  visitArg xobj@XObj{} = visit xobj
  unmanageArg :: XObj -> State MemState (Either TypeError XObj)
  unmanageArg xobj@(XObj _ _ (Just t)) = if isManaged typeEnv t
    then do
      r <- unmanage xobj
      case r of
        Left  err -> return (Left err)
  ...
  unmanageArg xobj@XObj{} = return (Right xobj)
  createDeleter :: XObj -> Maybe Deleter
  createDeleter xobj = case ty xobj of
  ...
  manage xobj = if isSymbolThatCaptures xobj -- When visiting lifted lambdas, don't manage symbols that capture (they are owned by the environment).
    then return ()
  ...
  isSymbolThatCaptures xobj = case xobj of
  ...
  unmanage xobj =
    let
      ...
      Just i = info xobj
    in
      if isManaged typeEnv t && not (isGlobalFunc xobj) && not
         (isExternalType typeEnv t)
      then
        do
          MemState deleters deps lifetimes <- get
          case deletersMatchingXObj xobj deleters of
            ...
            tooMany ->
              error
                (  "Too many variables with the same name in set: "
                ++ show tooMany
                )
      else
        return (Right ())
  ...
  refCheck xobj =
    let
      ...
      isGlobalVariable = case xobj of
        ...
        _ -> False
    in
      if not isGlobalVariable
           && not (isGlobalFunc xobj)
           && isManaged typeEnv t
           && not (isExternalType typeEnv t)
           && not (isSymbolThatCaptures xobj) -- TODO: The 'isManaged typeEnv t' boolean check should be removed!
        then do
          MemState deleters deps lifetimes <- get
          case deletersMatchingXObj xobj deleters of
            ...
            _ ->
              error
                $ "Too many variables with the same name in set (was looking for "
                ++ pretty xobj
                ++ " at "
                ++ prettyInfoFromXObj xobj
                ++ ")"
...
suffixTyVars suffix t = case t of
  VarTy key                -> VarTy (key ++ suffix)
  FuncTy argTys retTy ltTy -> FuncTy (map (suffixTyVars suffix) argTys)
                                     (suffixTyVars suffix retTy)
                                     (suffixTyVars suffix ltTy)
...
isGlobalFunc xobj = case xobj of
...
concreteDelete typeEnv env members = Template
  (FuncTy [VarTy "p"] UnitTy StaticLifetimeTy)
  (const (toTemplate "void $NAME($p p)"))
  (const
    (toTemplate $ unlines
      ["$DECL {", joinLines (map (memberDeletion typeEnv env) members), "}"]
    )
  )
  (\_ -> concatMap
    ( depsOfPolymorphicFunction typeEnv env [] "delete"
...
concreteDeleteTakePtr typeEnv env members = Template
  ...
  case
    ...
    of
      FunctionFound functionFullName ->
        "    " ++ functionFullName ++ "(p" ++ separator ++ memberName ++ ");"
      FunctionNotFound msg -> error msg
      FunctionIgnored ->
        "    /* Ignore non-managed member '"
          ++ memberName
          ++ "' : "
          ++ show memberType
          ++ " */"
...
concreteCopy typeEnv env memberPairs = Template
...
tokensForCopy typeEnv env memberPairs = toTemplate $ unlines
  [ "$DECL {"
  , "    $p copy = *pRef;"
...
concreteCopyPtr typeEnv env memberPairs = Template
  (FuncTy [RefTy (VarTy "p") (VarTy "q")] (VarTy "p") StaticLifetimeTy)
  (const (toTemplate "$p* $NAME($p* pRef)"))
...
tokensForCopyPtr typeEnv env memberPairs = toTemplate $ unlines
  [ "$DECL {"
