  ...
  where
    genF xobj args body captures =
      do
        ...
        let (argTys, retTy, lifetimeTy, fnRefLt) =
              case xobjType of
                (FuncTy a r l) -> (a, r, l, Nothing)
                (RefTy (FuncTy a r l) rlt) -> (a, r, l, Just rlt)
                _ -> error ("Invalid function type for " ++ pretty xobj ++ ": " ++ show xobjType)
            refConstr =
              case fnRefLt of
                Just fnRefLt' -> [Constraint fnRefLt' lifetimeTy xobj xobj xobj OrdFnRef]
                Nothing -> []
        ...
        pure (bodyConstr : argConstrs ++ insideBodyConstraints ++ capturesConstrs ++ sigConstr ++ refConstr)
    gen xobj =
