...
genConstraints
  :: Env -> XObj -> Maybe (Ty, XObj) -> Either TypeError [Constraint]
genConstraints globalEnv root rootSig = fmap sort (gen root)
 where
  genF xobj args body captures = do
    ...
    let (FuncTy argTys retTy lifetimeTy) = xobjType
        ...
        argConstrs = zipWith3
          (\a b aObj -> Constraint a b aObj xobj xobj OrdArg)
        ...
        sigConstr = if root == xobj
          then case rootSig of
            Just (rootSigTy, rootSigXObj) ->
              [ Constraint rootSigTy
                           xobjType
                           rootSigXObj
                           xobj
                           xobj
        ...
        capturesConstrs = mapMaybe
          id
          (zipWith
            (\captureTy captureObj -> case captureTy of
              RefTy _ refLt ->
                --trace ("Generated constraint between " ++ show lifetimeTy ++ " and " ++ show refLt) $
                               Just
                (Constraint lifetimeTy refLt captureObj xobj xobj OrdCapture)
              _ ->
  ...
  gen xobj = case obj xobj of
    Lst lst -> case lst of
      ...
      [XObj Def _ _, _, expr] -> do
        ...
        let defConstraint =
              Constraint xobjType exprType xobj expr xobj OrdDefExpr
      ...
      -- Let
      [XObj Let _ _, XObj (Arr bindings) _ _, body] -> do
        insideBodyConstraints <- gen body
        ...
        let Just xobjTy = ty xobj
            wholeStatementConstraint =
              Constraint bodyType xobjTy body xobj xobj OrdLetBody
            bindingsConstraints = zipWith
      ...
      -- If
      [XObj If _ _, expr, ifTrue, ifFalse] -> do
        ...
        let expected = XObj (Sym (SymPath [] "Condition in if value") Symbol)
                            (info expr)
                            (Just BoolTy)
        let
          lol = XObj (Sym (SymPath [] "lol") Symbol) (info expr) (Just BoolTy)
          conditionConstraint =
            Constraint exprType BoolTy expr expected xobj OrdIfCondition
          sameReturnConstraint =
            Constraint trueType falseType ifTrue ifFalse xobj OrdIfReturn
          Just t = ty xobj
          wholeStatementConstraint =
            Constraint trueType t ifTrue xobj xobj OrdIfWhole
        return
          (  conditionConstraint
          :  sameReturnConstraint
          :  wholeStatementConstraint
      ...
      -- Match
      XObj (Match matchMode) _ _ : expr : cases -> do
        insideExprConstraints <- gen expr
        casesLhsConstraints   <- fmap
          join
          (mapM (genConstraintsForCaseMatcher matchMode . fst) (pairwise cases))
        ...
        let -- Each case rhs should have the same return type as the whole match form:
          mkRetConstr x@(XObj _ _ (Just t)) =
            Just (Constraint t xobjType x xobj xobj OrdArg) -- TODO: Ord
          mkRetConstr _ = Nothing
          returnConstraints =
            mapMaybe (\(_, rhs) -> mkRetConstr rhs) (pairwise cases)
          -- Each case lhs should have the same type as the expression matching on
          mkExprConstr x@(XObj _ _ (Just t)) =
            Just
              (Constraint (wrapTyInRefIfMatchingRef t)
                          exprType
                          x
                          expr
                          xobj
          ...
          exprConstraints =
            mapMaybe (\(lhs, _) -> mkExprConstr lhs) (pairwise cases)
       ...
       where
        wrapTyInRefIfMatchingRef t = case matchMode of
          MatchValue -> t
          MatchRef   -> RefTy t (VarTy "whatever")
      -- While
      [XObj While _ _, expr, body] -> do
        insideConditionConstraints <- gen expr
        insideBodyConstraints <- gen body
        exprType <- toEither (ty expr) (ExpressionMissingType expr)
        bodyType <- toEither (ty body) (ExpressionMissingType body)
        let
          expectedCond = XObj
            (Sym (SymPath [] "Condition in while-expression") Symbol)
            (info expr)
            (Just BoolTy)
          expectedBody = XObj
            (Sym (SymPath [] "Body in while-expression") Symbol)
            (info xobj)
            (Just UnitTy)
          conditionConstraint =
            Constraint exprType BoolTy expr expectedCond xobj OrdWhileCondition
          wholeStatementConstraint =
            Constraint bodyType UnitTy body expectedBody xobj OrdWhileBody
        return
          (  conditionConstraint
          :  wholeStatementConstraint
          :  insideConditionConstraints
      ...
      -- Do
      XObj Do _ _ : expressions -> case expressions of
        ...
        _ ->
          ...
          in
            do
              ...
              xobjType <- toEither (ty xobj) (DefMissingType xobj)
              lastExprType <- toEither (ty lastExpr)
                                       (ExpressionMissingType xobj)
              let
                retConstraint = Constraint xobjType
                                           lastExprType
                                           xobj
                                           lastExpr
                                           xobj
                                           OrdDoReturn
                must = XObj
                  (Sym (SymPath [] "Statement in do-expression") Symbol)
                  (info xobj)
                  (Just UnitTy)
                mkConstr x@(XObj _ _ (Just t)) =
                  Just (Constraint t UnitTy x must xobj OrdDoStatement)
                mkConstr _ = Nothing
                expressionsShouldReturnUnit =
                  mapMaybe mkConstr (init expressions)
              return
                (  retConstraint
                :  insideExpressionsConstraints
                ++ expressionsShouldReturnUnit
                )

      -- Address
      [XObj Address _ _, value]           -> gen value
      -- Set!
      [XObj SetBang _ _, variable, value] -> do
        ...
        let sameTypeConstraint =
              Constraint variableType valueType variable value xobj OrdSetBang
        return
          (  sameTypeConstraint
          :  insideValueConstraints
          ++ insideVariableConstraints
          )
      ...
      [XObj The _ _, _, value] -> do
        ...
        let theTheConstraint =
              Constraint xobjType valueType xobj value xobj OrdThe
        return (theTheConstraint : insideValueConstraints)

      -- Ref
      [XObj Ref   _ _, value] -> gen value
      -- Deref
      [XObj Deref _ _, value] -> do
        ...
        let theTheConstraint =
              Constraint (RefTy xobjType lt) valueType xobj value xobj OrdDeref
        return (theTheConstraint : insideValueConstraints)

      -- Break
      [    XObj Break _ _]    -> return []
      ...
      func :             args -> do
        ...
        case funcTy of
          (FuncTy argTys retTy _) -> if length args /= length argTys
            then Left (WrongArgCount func (length argTys) (length args))
              ...
              let
                expected t n = XObj
                  ...
                  (info func)
                  (Just t)
                argConstraints = zipWith4
                  (\a t aObj n ->
                    Constraint a t aObj (expected t n) xobj OrdFuncAppArg
                  )
                  (List.map forceTy args)
                  argTys
                  args
                ...
                Just xobjTy = ty xobj
                retConstraint =
                  Constraint xobjTy retTy xobj func xobj OrdFuncAppRet
              in
                return
                  (  retConstraint
                  :  funcConstraints
          ...
          funcVarTy@(VarTy _) ->
            let
              fabricatedFunctionType =
                FuncTy (List.map forceTy args) (forceTy xobj) (VarTy "what?!")
              expected = XObj
                (Sym (SymPath [] ("Calling '" ++ getName func ++ "'")) Symbol)
                (info func)
                Nothing
              wholeTypeConstraint = Constraint funcVarTy
                                               fabricatedFunctionType
                                               func
                                               expected
                                               xobj
            ...
            in
              return
                (wholeTypeConstraint : funcConstraints ++ variablesConstraints)
        ...
        let
          ...
          headObj = XObj
            (Sym
              (SymPath
                []
                (  "I inferred the type of the array from its first element "
                ++ show (getPath x)
                )
              )
              Symbol
          ...
          betweenExprConstraints                       = zipWith
            ...
            [1 ..]
          headConstraint =
            Constraint headTy t headObj (genObj x 1) xobj OrdArrHead
        return
          (headConstraint : insideExprConstraints ++ betweenExprConstraints)

    -- THIS CODE IS VERY MUCH A DUPLICATION OF THE 'ARR' CODE FROM ABOVE:
    (StaticArr arr) -> case arr of
      []     -> Right []
      x : xs -> do
        insideExprConstraints <- fmap join (mapM gen arr)
        let
          ...
          headObj = XObj
            (Sym
              (SymPath
                []
                ("I inferred the type of the static array from its first element "
                ++ show (getPath x)
                )
              )
              Symbol
            ...
            (info x)
            (Just headTy)
          Just (RefTy (StructTy (ConcreteNameTy "StaticArray") [t]) _) =
            ty xobj
          betweenExprConstraints = zipWith
            (\o n -> Constraint headTy
                                (forceTy o)
...
genConstraintsForCaseMatcher
  :: MatchMode -> XObj -> Either TypeError [Constraint]
genConstraintsForCaseMatcher matchMode = gen
 where
  gen xobj@(XObj (Lst (caseName : variables)) _ _) = do
    ...
    case caseNameTy of
      (FuncTy argTys retTy _) -> if length variables /= length argTys
        then Left (WrongArgCount caseName (length argTys) (length variables)) -- TODO: This could be another error since this isn't an actual function call.
          ...
          let
            ...
            Just xobjTy = ty xobj
            retConstraint =
              Constraint xobjTy retTy xobj caseName xobj OrdFuncAppRet
          in
            return
              (  retConstraint
              :  caseNameConstraints
      ...
      funcVarTy@(VarTy _) ->
        let fabricatedFunctionType = FuncTy (List.map forceTy variables)
                                            (forceTy xobj)
                                            (VarTy "what?!") -- TODO: Fix
            expected = XObj
              (Sym (SymPath [] ("Matchin on '" ++ getName caseName ++ "'"))
                   Symbol
              )
              (info caseName)
              Nothing
            wholeTypeConstraint = Constraint funcVarTy
                                             ...
                                             OrdFuncAppVarTy
        in  return
              (wholeTypeConstraint : caseNameConstraints ++ variablesConstraints
              )
  ...
  refWrapper (XObj (Sym _ _) _ _) wrapThisType =
    wrapInRefTyIfMatchRef matchMode wrapThisType
  refWrapper _ t = t
