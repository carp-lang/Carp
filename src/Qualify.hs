...
setFullyQualifiedSymbols typeEnv globalEnv env (XObj (Lst [defn@(XObj (Defn _) _ _), sym@(XObj (Sym (SymPath _ functionName) _) _ _), args@(XObj (Arr argsArr) _ _), body]) i t)
  =
  ...
  -- Note: These inner envs is ephemeral since they are not stored in a module or global scope.
    let recursionEnv = Env Map.empty
                           (Just env)
                           (Just (functionName ++ "-recurse-env"))
                           []
                           RecursionEnv
                           0
    ...
    in  XObj
          (Lst
            ...
            , sym
            , args
            , setFullyQualifiedSymbols typeEnv globalEnv envWithArgs body
            ]
          )
          i
          t
setFullyQualifiedSymbols typeEnv globalEnv env (XObj (Lst [fn@(XObj (Fn _ _) _ _), args@(XObj (Arr argsArr) _ _), body]) i t)
  = let
...
setFullyQualifiedSymbols typeEnv globalEnv env (XObj (Lst [letExpr@(XObj Let _ _), bind@(XObj (Arr bindings) bindi bindt), body]) i t)
  | odd (length bindings)
  = XObj (Lst [letExpr, bind, body]) i t
  ...
  = let Just ii  = i
        lvl      = envFunctionNestingLevel env
        innerEnv = Env Map.empty
                       (Just env)
                       (Just ("let-env-" ++ show (infoIdentifier ii)))
                       []
                       InternalEnv
                       lvl
        (innerEnv', bindings') = foldl'
...
setFullyQualifiedSymbols typeEnv globalEnv env (XObj (Lst (matchExpr@(XObj (Match _) _ _) : expr : casesXObjs)) i t)
  = if even (length casesXObjs)
    then
      let
        newExpr  = setFullyQualifiedSymbols typeEnv globalEnv env expr
        Just ii  = i
        lvl      = envFunctionNestingLevel env
        innerEnv = Env Map.empty
                       (Just env)
                       (Just ("case-env-" ++ show (infoIdentifier ii)))
                       []
                       InternalEnv
                       lvl
        newCasesXObjs = map
                  ...
                  innerEnv' = foldl' folder innerEnv xs
                     where
                      folder e v = case v of
                        ...
                        XObj (Lst (y : ys)) _ _ -> foldl' folder innerEnv ys
                        x -> error ("Can't match variable with " ++ show x)
          ...
          (pairwise casesXObjs)
      in
        XObj (Lst (matchExpr : newExpr : concat newCasesXObjs)) i t
...
setFullyQualifiedSymbols typeEnv globalEnv localEnv xobj@(XObj (Sym path _) i t)
  = case path of
    -- Unqualified:
    SymPath [] name -> case lookupInEnv path (getTypeEnv typeEnv) of
      Just found ->
        ...
        Just (foundEnv, _) -> if envIsExternal foundEnv
      ...
      Nothing -> doesNotBelongToAnInterface False localEnv
    -- Qualified:
    _ -> doesNotBelongToAnInterface False localEnv
 where
  ...
  captureOrNot foundEnv =
    if envFunctionNestingLevel foundEnv < envFunctionNestingLevel localEnv
      then Capture
        (envFunctionNestingLevel localEnv - envFunctionNestingLevel foundEnv)
      else NoCapture
  doesNotBelongToAnInterface :: Bool -> Env -> XObj
  doesNotBelongToAnInterface finalRecurse theEnv =
    let
      results  = multiLookupQualified path theEnv
    ...
    in
      case results' of
        ...
        [(_, Binder _ foundOne@(XObj (Lst (XObj (External (Just overrideWithName)) _ _ : _)) _ _))]
          -> XObj
            (Sym (getPath foundOne) (LookupGlobalOverride overrideWithName))
            i
            t
        [(e, Binder _ foundOne)] -> case envMode e of
          ExternalEnv -> XObj
            (Sym
              (getPath foundOne)
              (LookupGlobal
                (if isExternalFunction foundOne then ExternalCode else CarpLand)
                (definitionMode foundOne)
              )
            )
            i
            t
        ...
        multiple -> case filter (not . envIsExternal . fst) multiple of
          -- There is at least one local binding, use the path of that one:
          (e, Binder _ local) : _ ->
            XObj (Sym (getPath local) (LookupLocal (captureOrNot e))) i t
          ...
          [] ->
            ...
            (SymPath [] name) ->
              -- Create a MultiSym!
                                 XObj
              (MultiSym name (map (getPath . binderXObj . snd) multiple))
              i
              t
            pathWithQualifiers ->
              -- The symbol IS qualified but can't be found, should produce an error later during compilation.
                                  trace
              ("PROBLEMATIC: " ++ show path)
              (XObj (Sym pathWithQualifiers (LookupGlobal CarpLand AFunction))
                    i
                    t
              )
  ...
  removeThoseShadowedByRecursiveSymbol allBinders = visit allBinders allBinders
   where
    visit bs result = foldl
      (\result b -> case b of
        (Env { envMode = RecursionEnv }, Binder _ xobj) -> remove
          (\(_, Binder _ x) -> xobj /= x && getName xobj == getName x)
          result
        _ -> result
      )
      result
