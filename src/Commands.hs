  ...
  do ctx <- get
     let proj = contextProj ctx
         ...
         newProj = case key of
                     ...
                     "no-compile" -> do noCompile <- unwrapBoolXObj value
                                        return (proj { projectNoCompile = noCompile })
...
commandProjectGetConfig [xobj@(XObj (Str key) _ _)] =
  ...
  where getVal ctx proj = case key of
          ...
          "no-compile" -> Right $ Str $ show (projectNoCompile proj)
          _ ->
...
commandHelp [] =
  liftIO $ do putStrLn "Compiler commands:"
              ...
              putStrLn "--no-compile                     - Don't run the compiler, just generate the C source."
              return dynamicNil
