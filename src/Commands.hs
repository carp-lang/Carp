  ...
  where
    ...
    withArity arity ctx args =
      if length args == arity
        ...
        else return (evalError ctx ("Invalid args to '" ++ show path ++ "' command: " ++ joinWithComma (map pretty args) ++ "\n\n" ++ exampleUsage) Nothing)
      ...
      do
        ...
        if generateOnly
          then return (ctx, dynamicNil)
            ...
            Just _ -> do
              let cmd = compiler ++ " " ++ outMain ++ " -o \"" ++ outExe ++ "\" " ++ flags
              liftIO $ do
            ...
            Nothing -> do
              let cmd = compiler ++ " " ++ outMain ++ " -shared -o \"" ++ outLib ++ "\" " ++ flags
              liftIO $ do
                when echoCompilationCommand (putStrLn cmd)
                callCommand cmd
...
commandIndexOf ctx [a, b] =
  ...
  where
    getIdx c s = fromIntegral $ fromMaybe (-1) $ elemIndex c s
