...
addCommand
  :: SymPath -> Int -> CommandCallback -> String -> String -> (String, Binder)
addCommand name arity callback doc example =
  addCommandConfigurable name (Just arity) callback doc example

addCommandConfigurable
  :: SymPath
  -> Maybe Int
  -> CommandCallback
  -> String
...
addCommandConfigurable path maybeArity callback doc example =
  let
    ...
    SymPath _ name = path
    meta           = MetaData
      (Map.insert "doc" (XObj (Str docString) Nothing Nothing) Map.empty)
  in
 ...
 where
  ...
  withArity arity ctx args = if length args == arity
...
presentError msg ret = liftIO $ do
  putStrLnWithColor Red msg
  return ret
...
commandProjectConfig ctx [xobj@(XObj (Str key) _ _), value] = do
  let
    ...
    newProj = case key of
      "cflag" -> do
        ...
        return
          (proj { projectCFlags = addIfNotPresent cflag (projectCFlags proj) })
      "libflag" -> do
        libflag <- unwrapStringXObj value
        return
          (proj
            { projectLibFlags = addIfNotPresent libflag (projectLibFlags proj)
            }
          )
      "pkgconfigflag" -> do
        ...
        return
          (proj
            { projectPkgConfigFlags = addIfNotPresent
                                        pkgconfigflag
                                        (projectPkgConfigFlags proj)
            }
          )
      "prompt" -> do
      ...
      "search-path" -> do
        ...
        return
          (proj
            { projectCarpSearchPaths = addIfNotPresent
                                         searchPath
                                         (projectCarpSearchPaths proj)
            }
          )
      "print-ast" -> do
      ...
      _ -> Left
        (  "Project.config can't understand the key '"
        ++ key
        ++ "' at "
  ...
  case newProj of
    Left errorMessage ->
      presentError ("[CONFIG ERROR] " ++ errorMessage) (ctx, dynamicNil)
    Right ok -> return (ctx { contextProj = ok }, dynamicNil)
commandProjectConfig ctx [faultyKey, _] = presentError
  ("First argument to 'Project.config' must be a string: " ++ pretty faultyKey)
  (ctx, dynamicNil)
...
commandProjectGetConfig ctx [xobj@(XObj (Str key) _ _)] =
  let
    ...
    getVal ctx proj = case key of
      ...
      "pkgconfigflag" ->
        Right $ Arr $ xstr . Str <$> projectPkgConfigFlags proj
      ...
      "file-path-print-length" ->
        Right $ Str $ show (projectFilePathPrintLength proj)
  ...
  in
    case getVal ctx proj of
      ...
      Left  key -> return
        (evalError
          ctx
          ("[CONFIG ERROR] Project.get-config can't understand the key '" ++ key
          )
          (info xobj)
        )
commandProjectGetConfig ctx [faultyKey] = presentError
  ("First argument to 'Project.config' must be a string: " ++ pretty faultyKey)
  (ctx, dynamicNil)

-- | Command for exiting the REPL/compiler
commandQuit :: CommandCallback
commandQuit ctx args = do
  liftIO exitSuccess
...
commandRunExe ctx args = do
  ...
  if projectCanExecute proj
    then liftIO $ do
      handle   <- spawnCommand outExe
      exitCode <- waitForProcess handle
      case exitCode of
        ExitSuccess ->
          return (ctx, Right (XObj (Num IntTy 0) (Just dummyInfo) (Just IntTy)))
        ExitFailure i -> throw
          (ShellOutException
            ("'" ++ outExe ++ "' exited with return value " ++ show i ++ ".")
            i
          )
    else liftIO $ do
      putStrLnWithColor
        Red
        "Can't call the 'run' command, need to build an executable first (requires a 'main' function)."
      return (ctx, dynamicNil)
...
commandBuild shutUp ctx args = do
  let
    ...
    src      = do
      ...
      initGlobals <- fmap (wrapInInitFunction (projectCore proj))
                          (globalsToC env)
      return
        (  "//Types:\n"
        ++ typeDecl
          ...
          Just _ -> do
            ...
            liftIO $ do
              ...
              when (execMode == Repl && not shutUp)
                   (putStrLn ("Compiled to '" ++ outExe ++ "' (executable)"))
          ...
          Nothing -> do
            ...
            liftIO $ do
              ...
              when
                ...
                (putStrLn ("Compiled to '" ++ outLib ++ "' (shared library)"))
...
commandListBindings ctx args = liftIO $ do
  putStrLn "Types:\n"
...
commandHelp ctx [XObj (Str "about") _ _] = liftIO $ do
  ...
  putStrLn ""
  return (ctx, dynamicNil)
commandHelp ctx [XObj (Str "language") _ _] = liftIO $ do
  putStrLn "Special forms:"
  ...
  putStrLn ""
  return (ctx, dynamicNil)
commandHelp ctx [XObj (Str "macros") _ _] = liftIO $ do
  putStrLn "To inspect what macros expand to:"
  ...
  putStrLn ""
  return (ctx, dynamicNil)
commandHelp ctx [XObj (Str "structs") _ _] = liftIO $ do
  putStrLn "To define a struct without any generic member variables:"
  ...
  putStrLn ""
  return (ctx, dynamicNil)
commandHelp ctx [XObj (Str "shortcuts") _ _] = liftIO $ do
  putStrLn "GHC-style shortcuts at the repl:"
  putStrLn "(reload)       :r"
  putStrLn "(build)        :b"
  putStrLn "(run)          :x"
  putStrLn "(cat)          :c"
  ...
  putStrLn ""
  return (ctx, dynamicNil)
commandHelp ctx [XObj (Str "interop") _ _] = liftIO $ do
  putStrLn
    "(register <name> <type>)                      - Make an external variable or function available for usage."
  putStrLn
    "(register-type <name> [<member> <type>, ...]) - Make an external struct available for usage."
  putStrLn ""
  ...
  putStrLn
    "(add-cflag <flag>)               - Add a cflag to the compilation step."
  putStrLn
    "(add-lib <flag>)                 - Add a library flag to the compilation step."
  return (ctx, dynamicNil)
commandHelp ctx [XObj (Str "project") _ _] = liftIO $ do
  putStrLn "(Project.config <setting> <value>) handles the following settings:"
  putStrLn ""
  ...
  putStrLn
    "'docs-url'             - A URL for the project (useful for generated documentation)."
  putStrLn
    "'docs-generate-index'  - Whether to generate the documentation index."
  putStrLn
    "'docs-styling'         - A URL to CSS for the project documentation."
  ...
  putStrLn
    "'paren-balance-hints'  - Whether to print the ongoing stack of parens to close in the REPL, or not."
  putStrLn
    "'force-reload'         - If true, the 'load-once' command will work just like 'load' (useful for library developers)."
  putStrLn ""
  return (ctx, dynamicNil)
commandHelp ctx [] = liftIO $ do
  putStrLn "Compiler commands:"
  putStrLn
    "(load <file>)      - Load a .carp file, evaluate its content, and add it to the project."
  putStrLn "(reload)           - Reload all the project files."
  putStrLn "(build)            - Produce an executable or shared library."
  putStrLn
    "(run)              - Run the executable produced by 'build' (if available)."
  putStrLn
    "(cat)              - Look at the generated C code (make sure you build first)."
  ...
  putStrLn
    "(help <chapter>)   - Available chapters: \"language\", \"macros\", \"structs\", \"interop\", \"shortcuts\", \"project\", \"about\"."
  putStrLn ""
  putStrLn "(Project.config! <setting> <value>) - Change a project setting."
  putStrLn ""
  putStrLn "To define things:"
  putStrLn
    "(def <name> <constant>)                       - Define a global variable."
  putStrLn "(defn <name> [<args>] <body>)                 - Define a function."
  putStrLn
    "(definterface <name> <type>)                  - Create an interface for a group of functions sharing the same name."
  putStrLn
    "(defmodule <name> <def1> <def2> ...)          - Define a module and/or add definitions to an existing one."
  putStrLn
    "(deftype <name> [<member> <type>, ...])       - Define a new struct type."
  putStrLn ""
  ...
  putStrLn
    "--log-memory                     - Enables use of memory logging functions in the Debug module."
  putStrLn
    "--optimize                       - Removes safety checks and runs the C-compiler with the '-O3' flag."
  putStrLn
    "--check                          - Report all errors found in a machine readable way."
  putStrLn "--generate-only                  - Don't compile the C source."
  return (ctx, dynamicNil)
commandHelp ctx args = return
  (evalError
    ctx
    ("Invalid args to `help` command: " ++ joinWithComma (map pretty args))
    Nothing
  )
...
commandAddInclude includerConstructor ctx [x] = case x of
  XObj (Str file) _ _ -> do
    let
      ...
      includers' =
        if includer `elem` includers then includers else includers ++ [includer] -- Add last to preserve include order
      proj' = proj { projectIncludes = includers' }
    return (ctx { contextProj = proj' }, dynamicNil)
  _ -> return
    (evalError
      ctx
      ("Argument to 'include' must be a string, but was `" ++ pretty x ++ "`")
      (info x)
    )
...
commandAddRelativeInclude ctx [x] = case x of
...
commandLength ctx [x] = case x of
  XObj (Lst lst) _ _ -> return
    ( ctx
    , (Right (XObj (Num IntTy (fromIntegral (length lst))) Nothing Nothing))
    )
  XObj (Arr arr) _ _ -> return
    ( ctx
...
commandAllButLast ctx [x] = case x of
  ...
  _                  -> return
    (evalError ctx "Applying 'all-but-last' to non-list or empty list." (info x)
    )

commandCons :: CommandCallback
commandCons ctx [x, xs] = case xs of
  XObj (Lst lst) _ _ ->
  ...
  _ -> return
    (evalError ctx "Applying 'cons' to non-list or empty list." (info xs))

commandConsLast :: CommandCallback
commandConsLast ctx [x, xs] = case xs of
  XObj (Lst lst) i t -> return (ctx, Right (XObj (Lst (lst ++ [x])) i t)) -- TODO: should they get their own i:s and t:s
  _                  -> return
    (evalError ctx "Applying 'cons-last' to non-list or empty list." (info xs))

commandAppend :: CommandCallback
commandAppend ctx [xs, ys] = case (xs, ys) of
  ...
  _ -> return
    (evalError ctx
               "Applying 'append' to non-array/list or empty list."
               (info xs)
    )

commandMacroError :: CommandCallback
commandMacroError ctx [msg] = case msg of
...
commandEq ctx [a, b] = return $ case (a, b) of
  ...
  (XObj (Lst []) _ _, XObj (Lst []) _ _) -> (ctx, Right trueXObj)
  _ -> evalError ctx
                 ("Can't compare " ++ pretty a ++ " with " ++ pretty b)
                 (info a)

commandLt :: CommandCallback
commandLt ctx [a, b] = return $ case (a, b) of
  (XObj (Num IntTy aNum) _ _, XObj (Num IntTy bNum) _ _) ->
    if (round aNum :: Int) < (round bNum :: Int)
      then (ctx, Right trueXObj)
      else (ctx, Right falseXObj)
...
commandGt ctx [a, b] = return $ case (a, b) of
  (XObj (Num IntTy aNum) _ _, XObj (Num IntTy bNum) _ _) ->
    if (round aNum :: Int) > (round bNum :: Int)
      then (ctx, Right trueXObj)
      else (ctx, Right falseXObj)
...
commandCharAt ctx [a, b] = return $ case (a, b) of
...
commandStringLength ctx [a] = return $ case a of
  XObj (Str s) _ _ ->
    ( ctx
    , Right
      (XObj (Num IntTy (fromIntegral (length s))) (Just dummyInfo) (Just IntTy))
    )
  _ -> evalError ctx ("Can't call length with " ++ pretty a) (info a)

commandStringConcat :: CommandCallback
commandStringConcat ctx [a] = return $ case a of
  XObj (Arr strings) _ _ -> case mapM unwrapStringXObj strings of
    Left err -> evalError ctx err (info a)
    Right result ->
      (ctx, Right (XObj (Str (join result)) (Just dummyInfo) (Just StringTy)))
  _ -> evalError ctx ("Can't call concat with " ++ pretty a) (info a)

commandSymConcat :: CommandCallback
commandSymConcat ctx [a] = return $ case a of
  XObj (Arr syms) _ _ -> case mapM unwrapSymPathXObj syms of
    Left err -> evalError ctx err (info a)
    Right result ->
      ( ctx
      , Right
        (XObj
          (Sym (SymPath [] (join (map show result)))
               (LookupGlobal CarpLand AVariable)
          )
          (Just dummyInfo)
          Nothing
        )
...
commandSymPrefix ctx [XObj (Sym (SymPath [] prefix) _) _ _, XObj (Sym (SymPath [] suffix) _) i t]
  = return
    $ ( ctx
      , Right
        (XObj
          (Sym (SymPath [prefix] suffix) (LookupGlobal CarpLand AVariable))
          i
          t
        )
      )
...
commandSymFrom ctx [XObj (Str s) i t] =
  return (ctx, Right $ XObj (sFrom_ s) i t)
commandSymFrom ctx [XObj (Pattern s) i t] =
  return (ctx, Right $ XObj (sFrom_ s) i t)
commandSymFrom ctx [XObj (Chr c) i t] =
  return (ctx, Right $ XObj (sFrom_ (show c)) i t)
commandSymFrom ctx [XObj n@(Num _ _) i t] =
  return (ctx, Right $ XObj (sFrom_ (simpleFromNum n)) i t)
commandSymFrom ctx [XObj (Bol b) i t] =
  return (ctx, Right $ XObj (sFrom_ (show b)) i t)
...
commandStringDirectory ctx [a] = return $ case a of
  XObj (Str s) _ _ ->
...
commandPlus ctx [a, b] = return $ case (a, b) of
  (XObj (Num aty aNum) _ _, XObj (Num bty bNum) _ _) -> if aty == bty
    then (ctx, Right (XObj (Num aty (aNum + bNum)) (Just dummyInfo) (Just aty)))
    else evalError ctx
                   ("Can't call + with " ++ pretty a ++ " and " ++ pretty b)
                   (info a)
  _ -> evalError ctx
                 ("Can't call + with " ++ pretty a ++ " and " ++ pretty b)
                 (info a)
...
commandMinus ctx [a, b] = return $ case (a, b) of
  (XObj (Num aty aNum) _ _, XObj (Num bty bNum) _ _) -> if aty == bty
    then (ctx, Right (XObj (Num aty (aNum - bNum)) (Just dummyInfo) (Just aty)))
    else evalError ctx
                   ("Can't call - with " ++ pretty a ++ " and " ++ pretty b)
...
commandDiv ctx [a, b] = return $ case (a, b) of
  (XObj (Num IntTy aNum) _ _, XObj (Num IntTy bNum) _ _) ->
    ( ctx
    , Right
      (XObj
        (Num IntTy (fromIntegral (quot (round aNum :: Int) (round bNum :: Int)))
        )
        (Just dummyInfo)
        (Just IntTy)
      )
...
commandMul ctx [a, b] = return $ case (a, b) of
  (XObj (Num aty aNum) _ _, XObj (Num bty bNum) _ _) -> if aty == bty
    then (ctx, Right (XObj (Num aty (aNum * bNum)) (Just dummyInfo) (Just aty)))
    else evalError ctx
                   ("Can't call * with " ++ pretty a ++ " and " ++ pretty b)
...
commandStr ctx xs = return
  (ctx, Right (XObj (Str (join (map f xs))) (Just dummyInfo) (Just StringTy)))
 where
...
commandNot ctx [x] = case x of
  XObj (Bol ab) _ _ ->
    if ab then return (ctx, Right falseXObj) else return (ctx, Right trueXObj)
  _ -> return
    (evalError ctx
               ("Can't perform logical operation (not) on " ++ pretty x)
               (info x)
    )

commandReadFile :: CommandCallback
commandReadFile ctx [filename] = case filename of
  XObj (Str fname) _ _ -> do
    exceptional <- liftIO
      ((try $ slurp fname) :: (IO (Either IOException String)))
    case exceptional of
      Right contents -> return
        (ctx, Right (XObj (Str contents) (Just dummyInfo) (Just StringTy)))
      Left _ -> return
        (evalError
          ctx
          ("The argument to `read-file` `" ++ fname ++ "` does not exist")
          (info filename)
        )
  _ -> return
    (evalError
      ctx
      (  "The argument to `read-file` must be a string, I got `"
      ++ pretty filename
      ++ "`"
      )
      (info filename)
...
commandWriteFile ctx [filename, contents] = case filename of
  XObj (Str fname) _ _ -> case contents of
    XObj (Str s) _ _ -> do
      exceptional <- liftIO
        ((try $ writeFile fname s) :: (IO (Either IOException ())))
      case exceptional of
        Right () -> return (ctx, dynamicNil)
        Left  _  -> return
          (evalError
            ctx
            (  "Cannot write to argument to `"
            ++ fname
            ++ "`, an argument to `write-file`"
            )
            (info filename)
          )
    _ -> return
      (evalError
        ctx
        (  "The second argument to `write-file` must be a string, I got `"
        ++ pretty contents
        ++ "`"
        )
        (info contents)
  ...
  _ -> return
    (evalError
      ctx
      (  "The first argument to `write-file` must be a string, I got `"
      ++ pretty filename
      ++ "`"
      )
      (info filename)
...
commandBitWidth ctx [] =
  ...
  in  return
        (ctx, Right (XObj (Num IntTy bitSize) (Just dummyInfo) (Just IntTy)))

commandSaveDocsInternal :: CommandCallback
commandSaveDocsInternal ctx [modulePath] = do
  let globalEnv = contextGlobalEnv ctx
  case modulePath of
    XObj (Lst xobjs) _ _ -> case mapM unwrapSymPathXObj xobjs of
      Left err -> return (evalError ctx err (info modulePath))
      Right okPaths ->
        case
            mapM (getEnvironmentBinderForDocumentation ctx globalEnv) okPaths
          of
            Left  err          -> return (evalError ctx err (info modulePath))
    ...
    x -> return
      (evalError
        ctx
        (  "Invalid arg to save-docs-internal (expected list of symbols): "
        ++ pretty x
        )
        (info modulePath)
      )
 where
  getEnvironmentBinderForDocumentation
    :: Context -> Env -> SymPath -> Either String Binder
  getEnvironmentBinderForDocumentation ctx env path =
    case lookupInEnv path env of
      Just (_, foundBinder@(Binder _ (XObj (Mod foundEnv) _ _))) ->
        Right foundBinder
      Just (_, Binder _ x) -> Left
        (  "I can’t generate documentation for `"
        ++ pretty x
        ++ "` because it isn’t a module"
        )
      Nothing -> Left ("I can’t find the module `" ++ show path ++ "`")
...
commandSexpression ctx [xobj] = commandSexpressionInternal ctx [xobj] False
commandSexpression ctx xobj   = return $ evalError
  ctx
  (  "s-expr expects a symbol argument and an optional bool, but got: "
  ++ unwords (map pretty xobj)
  )
  (Just dummyInfo)

commandSexpressionInternal
  :: Context -> [XObj] -> Bool -> IO (Context, Either EvalError XObj)
commandSexpressionInternal ctx [xobj] bol =
  ...
  in
    case xobj of
      (XObj (Lst [inter@(XObj (Interface ty _) _ _), path]) i t) -> return
        (ctx, Right (XObj (Lst [(toSymbols inter), path, (tyToXObj ty)]) i t))
      (XObj (Lst forms) i t) ->
        return (ctx, Right (XObj (Lst (map toSymbols forms)) i t))
      mod@(XObj (Mod e) i t) -> if bol
        then getMod
        else
          case
            lookupInEnv (SymPath [] (fromMaybe "" (envModuleName e))) tyEnv
          of
            Just (_, Binder _ (XObj (Lst forms) i t)) ->
              return (ctx, Right (XObj (Lst (map toSymbols forms)) i t))
            Just (_, Binder _ xobj') -> return (ctx, Right (toSymbols xobj'))
            Nothing                  -> getMod
       where
        getMod = case (toSymbols mod) of
          x@(XObj (Lst xs) i t) -> bindingSyms e (ctx, Right x)
         where
          ...
          combine _             (c, (Left err)) = (c, Left err)
          combine (c, Left err) _               = (c, Left err)
      _ -> return $ evalError
        ctx
        (  "can't get an s-expression for: "
        ++ pretty xobj
        ++ " is it a bound symbol or literal s-expression?"
        )
        (Just dummyInfo)
...
toSymbols (XObj (Deftype _) i t) =
  (XObj (Sym (SymPath [] "deftype") Symbol) i t)
toSymbols (XObj (DefSumtype _) i t) =
  (XObj (Sym (SymPath [] "deftype") Symbol) i t)
toSymbols (XObj (Interface _ _) i t) =
  (XObj (Sym (SymPath [] "definterface") Symbol) i t)
