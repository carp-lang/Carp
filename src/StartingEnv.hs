...
templatePointerCopy = defineTemplate
  ...
  (FuncTy [RefTy (PointerTy (VarTy "p")) (VarTy "q")]
          (PointerTy (VarTy "p"))
          StaticLifetimeTy
  )
  "copies a pointer `p`."
  (toTemplate "$p* $NAME ($p** ptrRef)")
  (toTemplate $ unlines ["$DECL {", "    return *ptrRef;", "}"])
  (const [])
...
functionModule = Env { envBindings             = bindings
 ...
 where
  ...
  bindings =
    Map.fromList (map (bindEnv . generateInnerFunctionModule) [0 .. maxArity])
...
generateInnerFunctionModule arity = Env
 ...
 where
  alphabet = ['d' .. 'y']
  charToTyName c = [c]
  funcTy = FuncTy (take arity (map (VarTy . charToTyName) alphabet))
                  (VarTy "z")
                  StaticLifetimeTy
  bindings = Map.fromList
    [ generateTemplateFuncCopy funcTy
    , generateTemplateFuncDelete funcTy
    , generateTemplateFuncStrOrPrn "str"
                                   "converts a function to a string."
                                   funcTy
    , generateTemplateFuncStrOrPrn
      "prn"
      "converts a function to a string (internal representation)."
      funcTy
    ]
...
dynamicModule = Env { envBindings             = bindings
 ...
 where
  ...
  bindings =
    Map.fromList
      $  [ addCommand (SymPath path "list?")
                      1
                      commandIsList
                      "checks whether the argument is a list."
                      "(list? '()) ; => true"
         , addCommand (SymPath path "array?")
                      1
                      commandIsArray
                      "checks whether the arguments is an array."
                      "(array? []) ; => true"
         , addCommand (SymPath path "symbol?")
                      1
                      commandIsSymbol
                      "checks whether the argument is a symbol."
                      "(symbol? 'x) ; => true"
         , addCommand
           (SymPath path "length")
           1
         ...
         , addCommand (SymPath path "*")
                      ...
                      "(* 2 3) ; => 6"
         , addCommand (SymPath path "c")
                      1
                      commandC
                      "prints the C code emitted for a binding."
                      "(c '(+ 2 3)) ; => int _3 = Int__PLUS_(2, 3);"
         , addCommand (SymPath path "quit")
                      0
                      commandQuit
                      "quits the program."
                      "(quit)"
         , addCommand (SymPath path "cat")
                      0
                      commandCat
                      "spits out the generated C code."
                      "(cat)"
         , addCommand (SymPath path "run")
                      0
                      commandRunExe
                      "runs the built executable."
                      "(run)"
         , addCommand (SymPath path "build")
                      0
                      (commandBuild False)
                      "builds the current code to an executable."
                      "(build)"
         , addCommand
           (SymPath path "reload")
           0
         ...
         , addCommand
           ...
           0
           commandOS
           "prints the operating system (as returned by the Haskell function `System.Info.os`)."
           "(os)"
         , addCommand
           (SymPath path "system-include")
           1
           commandAddSystemInclude
           "adds a system include, i.e. a C `#include` with angle brackets (`<>`)."
           "(system-include \"stdint.h\")"
         , addCommand
           (SymPath path "relative-include")
           1
           commandAddRelativeInclude
           "adds a relative include, i.e. a C `include` with quotes. It also prepends the current directory."
           "(relative-include \"myheader.h\")"
         , addCommand
           (SymPath path "save-docs-internal")
           1
           commandSaveDocsInternal
           "is the internal companion command to `save-docs`. `save-docs` should be called instead."
           "(save-docs-internal 'Module)"
         , addCommand (SymPath path "read-file")
                      1
                      commandReadFile
                      "reads a file into a string."
                      "(read-file \"myfile.txt\")"
         , addCommand (SymPath path "write-file")
                      2
                      commandWriteFile
                      "writes a string to a file."
                      "(write-file \"myfile\" \"hello there!\")"
         , addCommand
           (SymPath path "bit-width")
           0
           commandBitWidth
           "gets the bit width of the platform."
           "(bit-width) ; => your host machine’s bit width, e.g. 32 or 64"
         , addCommandConfigurable
           (SymPath path "s-expr")
           Nothing
           commandSexpression
      ...
      ++ [ ( "String"
           , Binder emptyMeta (XObj (Mod dynamicStringModule) Nothing Nothing)
           )
         , ( "Symbol"
           , Binder emptyMeta (XObj (Mod dynamicSymModule) Nothing Nothing)
           )
         , ( "Project"
           , Binder emptyMeta (XObj (Mod dynamicProjectModule) Nothing Nothing)
           )
         ]
...
dynamicStringModule = Env { envBindings             = bindings
 ...
 where
  ...
  bindings = Map.fromList
    ...
    , addCommand (SymPath path "length")
                 1
                 commandStringLength
                 "gets the length of a string."
                 "(String.length \"hi\") ; => 2"
    , addCommand (SymPath path "concat")
                 1
                 commandStringConcat
                 "concatenates a list of strings together."
                 "(String.concat [\"hi \" \"there\"]) ; => \"hi there\""
    , addCommand
      (SymPath path "directory")
      1
...
dynamicProjectModule = Env { envBindings             = bindings
 ...
 where
  ...
  bindings = Map.fromList
    [ addCommand (SymPath path "config")
                 2
                 commandProjectConfig
                 "sets a project config key."
                 "(Project.config \"paren-balance-hints\" false)"
    , addCommand (SymPath path "get-config")
                 1
                 commandProjectGetConfig
                 "gets a project config value under a key."
                 "(Project.get-config \"paren-balance-hints\")"
    ]

-- | The global environment before any code is run.
startingGlobalEnv :: Bool -> Env
startingGlobalEnv noArray = Env { envBindings             = bindings
                                , envParent               = Nothing
    ...
    Map.fromList
      ...
      ++ (if noArray
           then []
             ...
             [ ( "Array"
               , Binder emptyMeta (XObj (Mod arrayModule) Nothing Nothing)
               )
             ]
         )
      ++ [ ( "StaticArray"
           , Binder emptyMeta (XObj (Mod staticArrayModule) Nothing Nothing)
           )
         ]
      ++ [ ( "Pointer"
           , Binder emptyMeta (XObj (Mod pointerModule) Nothing Nothing)
           )
         ]
      ++ [ ( "Dynamic"
           , Binder emptyMeta (XObj (Mod dynamicModule) Nothing Nothing)
           )
         ]
      ++ [ ( "Function"
           , Binder emptyMeta (XObj (Mod functionModule) Nothing Nothing)
           )
         ]
 ...
 where
  bindings = Map.fromList
    [ interfaceBinder
      ...
      (  [SymPath ["Array"] "copy", SymPath ["Pointer"] "copy"]
      ++ registerFunctionFunctionsWithInterface "copy"
      )
      builtInSymbolInfo
    , interfaceBinder
      "str"
      ...
      ( (SymPath ["Array"] "str")
      : (SymPath ["StaticArray"] "str")
      : registerFunctionFunctionsWithInterface "str"
      )
      builtInSymbolInfo
    , interfaceBinder
      "prn"
      ...
      ) -- QUESTION: Where is 'prn' for dynamic Array:s registered? Can't find it... (but it is)
      builtInSymbolInfo
    ]
...
registerFunctionFunctionsWithInterface interfaceName = map
  (\arity -> SymPath ["Function", "Arity" ++ show arity] interfaceName)
  [0 .. maxArity]

-- | Create a binder for an interface definition.
interfaceBinder :: String -> Ty -> [SymPath] -> Info -> (String, Binder)
interfaceBinder name t paths i =
  (name, Binder emptyMeta (defineInterface name t paths (Just i)))
