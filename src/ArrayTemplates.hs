...
templateEMap =
  let
    fTy  = FuncTy [VarTy "a"] (VarTy "a") (VarTy "fq")
  ...
  in
    defineTemplate
      ...
      (toTemplate $ unlines
        ...
        , "        (($a*)a.data)[i] = "
        ++ templateCodeForCallingLambda "(*f)" fTy [elem]
        ++ ";"
        , "    }"
        , "    return a;"
        , "}"
      ...
      (\(FuncTy [RefTy t@(FuncTy fArgTys fRetTy _) _, arrayType] _ _) ->
        ...
        , defineFunctionTypeAlias
          (FuncTy (lambdaEnvTy : fArgTys) fRetTy StaticLifetimeTy)
        ]
      )

templateShrinkCheck :: String -> String
templateShrinkCheck var = unlines
  ...
  templateCreator = TemplateCreator $ \typeEnv env -> Template
      ...
      toTemplate
        ...
        $ let deleter = insideArrayDeletion typeEnv env insideTy
          in  [ "$DECL { "
              ...
              , "    for(int i = 0; i < a.len; ++i) {"
              , "        if("
              ++ templateCodeForCallingLambda "(*predicate)" fTy [elem]
              ++ ") {"
    ...
    (\(FuncTy [RefTy ft@(FuncTy fArgTys@[RefTy insideType _] BoolTy _) _, arrayType] _ _) ->
      [ defineFunctionTypeAlias ft
        , defineFunctionTypeAlias
          (FuncTy (lambdaEnvTy : fArgTys) BoolTy StaticLifetimeTy)
        ]
...
templateNth =
  ...
  in  defineTemplate
        ...
        (FuncTy
          [RefTy (StructTy (ConcreteNameTy "Array") [t]) (VarTy "q"), IntTy]
          (RefTy t (VarTy "q"))
          StaticLifetimeTy
        )
...
templateRaw = defineTemplate
  (SymPath ["Array"] "raw")
  (FuncTy [StructTy (ConcreteNameTy "Array") [VarTy "t"]]
          (PointerTy (VarTy "t"))
          StaticLifetimeTy
  )
...
templateUnsafeRaw = defineTemplate
  ...
  (FuncTy [RefTy (VarTy "q") (StructTy (ConcreteNameTy "Array") [VarTy "t"])]
          (PointerTy (VarTy "t"))
          StaticLifetimeTy
  )
  "returns an array `a` as a raw pointer—useful for interacting with C."
...
templateAset = defineTypeParameterizedTemplate templateCreator path t docs
 where
  ...
  t    = FuncTy
    [StructTy (ConcreteNameTy "Array") [VarTy "t"], IntTy, VarTy "t"]
    (StructTy (ConcreteNameTy "Array") [VarTy "t"])
    StaticLifetimeTy
  ...
  templateCreator = TemplateCreator $ \typeEnv env -> Template
    ...
    (\(FuncTy [_, _, insideTy] _ _) -> depsForDeleteFunc typeEnv env insideTy)
...
templateAsetBang = defineTypeParameterizedTemplate templateCreator path t docs
 where
  path = SymPath ["Array"] "aset!"
  t    = FuncTy
    [ RefTy (StructTy (ConcreteNameTy "Array") [VarTy "t"]) (VarTy "q")
    , IntTy
    , VarTy "t"
    ]
    UnitTy
...
templateAsetUninitializedBang = defineTypeParameterizedTemplate
  templateCreator
  path
  t
  docs
...
templateAllocate = defineTypeParameterizedTemplate templateCreator path t docs
 where
  ...
  t    = FuncTy [IntTy]
                ...
                StaticLifetimeTy
  docs
    = "allocates an uninitialized array. You can initialize members using [`aset-uninitialized`](#aset-uninitialized)."
  templateCreator = TemplateCreator $ \typeEnv env -> Template
    t
    (const (toTemplate "Array $NAME (int n)"))
    (\(FuncTy [_] arrayType _) -> toTemplate $ unlines
      (  [ "$DECL {"
         , "    Array a;"
         , "    a.len = n;"
         , "    a.capacity = n;"
    ...
    (\(FuncTy [_] arrayType _) -> depsForDeleteFunc typeEnv env arrayType)

templateDeleteArray :: (String, Binder)
templateDeleteArray = defineTypeParameterizedTemplate templateCreator
                                                      path
                                                      t
                                                      docs
 where
  path = SymPath ["Array"] "delete"
  t    = FuncTy [StructTy (ConcreteNameTy "Array") [VarTy "a"]]
                UnitTy
                StaticLifetimeTy
  docs =
...
insideArrayDeletion typeEnv env t indexer =
  case
    ...
    of
      ...
      FunctionIgnored ->
        "    /* Ignore non-managed type inside Array: '" ++ show t ++ "' */\n"
...
templateCopyArray = defineTypeParameterizedTemplate templateCreator path t docs
 where
  ...
  t    = FuncTy
    [RefTy (StructTy (ConcreteNameTy "Array") [VarTy "a"]) (VarTy "q")]
    (StructTy (ConcreteNameTy "Array") [VarTy "a"])
    StaticLifetimeTy
  ...
  templateCreator = TemplateCreator $ \typeEnv env -> Template
    ...
    (\(FuncTy [RefTy arrayType _] _ _) ->
      [TokDecl, TokC "{\n"]
        ...
        ++ [ TokC "    copy.data = CARP_MALLOC(sizeof("
           , TokTy (VarTy "a") Normal
           , TokC ") * a->capacity);\n"
           ]
...
copyTy typeEnv env (StructTy (ConcreteNameTy "Array") [innerType]) =
  if isManaged
    then
      ...
      [ TokC "    memcpy(copy.data, a->data, sizeof("
      , TokTy (VarTy "a") Normal
      , TokC ") * a->len);\n"
      ]
 where
...
insideArrayCopying typeEnv env t =
  case
      findFunctionForMemberIncludePrimitives typeEnv
                                             env
                                             "copy"
                                             (typesCopyFunctionType t)
                                             ("Inside array.", t)
    of
      FunctionFound functionFullName ->
        "    (("
      ...
      FunctionNotFound msg -> error msg
      FunctionIgnored ->
        "    /* Ignore type inside Array when copying: '"
          ++ show t
          ++ "' (no copy function known)*/\n"
...
calculateStrSize typeEnv env t = unlines
  ...
  arrayMemberSizeCalc =
    case
        findFunctionForMemberIncludePrimitives
          ...
          env
          "prn"
          (typesStrFunctionType typeEnv t)
          ("Inside array.", t)
      of
        FunctionFound functionFullName ->
        ...
        FunctionNotFound msg -> error msg
        FunctionIgnored ->
          "    /* Ignore type inside Array: '" ++ show t ++ "' ??? */\n"
