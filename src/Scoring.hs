 ...
 where
  depthOfStruct (StructTy (ConcreteNameTy structName) varTys) =
    case lookupInEnv (SymPath [] structName) (getTypeEnv typeEnv) of
      Just (_, Binder _ typedef) ->
        (depthOfDeftype typeEnv Set.empty typedef varTys + 1, b)
      Nothing -> error
        ("Can't find user defined type '" ++ structName ++ "' in type env.")
scoreTypeBinder _ b@(Binder _ (XObj (Mod _) _ _)) = (1000, b)
 ...
 where
  ...
  expandCase (XObj (Arr arr) _ _) =
    let members = memberXObjsToPairs arr
        depthsFromMembers =
            map (depthOfType typeEnv visited selfName . snd) members
    in  depthsFromMembers ++ depthsFromVarTys
  expandCase (XObj (Lst [XObj{}, XObj (Arr sumtypeCaseTys) _ _]) _ _) =
    let depthsFromCaseTys = map
          (depthOfType typeEnv visited selfName . fromJust . xobjToTy)
          sumtypeCaseTys
...
depthOfType typeEnv visited selfName theType = if theType `elem` visited
  then 0
 ...
 where
  ...
  depthOfStructType name varTys = 1 + case name of
    "Array" -> depthOfVarTys
    _
      | name == selfName -> 1
      | otherwise -> case lookupInEnv (SymPath [] name) (getTypeEnv typeEnv) of
        Just (_, Binder _ typedef) ->
   ...
   where
    depthOfVarTys = case fmap (depthOfType typeEnv visited name) varTys of
      [] -> 1
      xs -> maximum xs + 1
...
scoreValueBinder globalEnv visited binder@(Binder _ (XObj (Lst [XObj (Defn _) _ _, XObj (Sym path Symbol) _ _, _, body]) _ _))
  = (scoreBody globalEnv visited body, binder)
scoreValueBinder _ _ binder = (0, binder)
...
scoreBody globalEnv visited root = visit root
 where
  visit xobj = case obj xobj of
    ...
    (Sym path (LookupGlobal _ _)) -> if Set.member path visited
      then 0
      else case lookupInEnv path globalEnv of
        Just (_, foundBinder) ->
          let (score, _) = scoreValueBinder globalEnv
                                            (Set.insert path visited)
                                            foundBinder
          in  score + 1
        Nothing -> error ("Failed to lookup '" ++ show path ++ "'.")
    _ -> 0
  visitList (XObj (Lst []   ) _ _) = 0
  visitList (XObj (Lst xobjs) _ _) = maximum (fmap visit xobjs)
  visitArray (XObj (Arr []   ) _ _) = 0
  visitArray (XObj (Arr xobjs) _ _) = maximum (fmap visit xobjs)
