...
validateMemberCases typeEnv typeVariables rest = mapM_ visit rest
 where
  ...
  visit xobj = Left (InvalidSumtypeCase xobj)

validateMembers :: TypeEnv -> [Ty] -> [XObj] -> Either TypeError ()
validateMembers typeEnv typeVariables membersXObjs =
  if length membersXObjs `mod` 2 == 0
    then mapM_ (okXObjForType typeEnv typeVariables . snd)
               (pairwise membersXObjs)
    else Left (UnevenMembers membersXObjs)

validateOneCase _ XObj{} =
  error
    "Type members must be defined using array syntax: [member1 type1 member2 type2 ...]" -- TODO: How to reach this case?

okXObjForType :: TypeEnv -> [Ty] -> XObj -> Either TypeError ()
okXObjForType typeEnv typeVariables xobj = case xobjToTy xobj of
...
canBeUsedAsMemberType typeEnv typeVariables t xobj = case t of
  IntTy           -> return ()
  FloatTy         -> return ()
  DoubleTy        -> return ()
  ByteTy          -> return ()
  ...
  StructTy (ConcreteNameTy "Array") [inner] -> do
    _ <- canBeUsedAsMemberType typeEnv typeVariables inner xobj
    return ()
  StructTy name [tyVars] -> case name of
    (ConcreteNameTy name') ->
    ...
    t@(VarTy _) -> return ()
  s@(StructTy name tyvar) -> if isExternalType typeEnv s
    then return ()
    else case name of
      (ConcreteNameTy n) ->
       ...
       where
        ...
        checkInhabitants ty = case ty of
          (StructTy _ vars) -> if length vars == length tyvar
            then return ()
            else Left
              (UninhabitedConstructor ty xobj (length tyvar) (length vars))
