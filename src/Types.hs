...
typeEqIgnoreLifetimes (StructTy a tyVarsA) (StructTy b tyVarsB) =
  a == b && all (== True) (zipWith typeEqIgnoreLifetimes tyVarsA tyVarsB)
...
instance Show Ty where
  ...
  show CharTy    = "Char"
  show (FuncTy argTys retTy StaticLifetimeTy) =
    "("
      ++ fnOrLambda
      ++ " ["
      ++ joinWithComma (map show argTys)
      ++ "] "
      ++ show retTy
      ++ ")"
  show (FuncTy argTys retTy lt) =
    "("
      ++ fnOrLambda
      ++ " ["
      ++ joinWithComma (map show argTys)
  ...
  show (StructTy s typeArgs) =
    "(" ++ (show s) ++ " " ++ joinWithSpace (map show typeArgs) ++ ")"
...
isTypeGeneric (FuncTy argTys retTy _) =
  any isTypeGeneric argTys || isTypeGeneric retTy
...
doesTypeContainTyVarWithName name (StructTy n tyArgs) =
  doesTypeContainTyVarWithName name n
    || any (doesTypeContainTyVarWithName name) tyArgs
doesTypeContainTyVarWithName name (PointerTy p) =
  doesTypeContainTyVarWithName name p
doesTypeContainTyVarWithName name (RefTy r lt) =
  doesTypeContainTyVarWithName name r || doesTypeContainTyVarWithName name lt
doesTypeContainTyVarWithName _ _ = False

replaceConflicted :: String -> Ty -> Ty
replaceConflicted name (VarTy n) =
  if n == name then (VarTy (n ++ "conflicted")) else (VarTy n)
replaceConflicted name (FuncTy argTys retTy lt) = FuncTy
  (map (replaceConflicted name) argTys)
  (replaceConflicted name retTy)
  (replaceConflicted name lt)
replaceConflicted name (StructTy n tyArgs) =
  StructTy (replaceConflicted name n) (map (replaceConflicted name) tyArgs)
replaceConflicted name (PointerTy p) = PointerTy (replaceConflicted name p)
replaceConflicted name (RefTy r lt) =
  RefTy (replaceConflicted name r) (replaceConflicted name lt)
...
unifySignatures v t = Map.fromList (unify v t)
 where
  ...
  unify (StructTy v@(VarTy a) aArgs) (StructTy n bArgs) =
    unify v n ++ concat (zipWith unify aArgs bArgs)
...
areUnifiable (StructTy a aArgs) (StructTy b bArgs)
  ...
  = False
  | areUnifiable a b
  = let argBools = zipWith areUnifiable aArgs bArgs in all (== True) argBools
  | otherwise
  = False
...
areUnifiable (RefTy a ltA) (RefTy b ltB) =
  areUnifiable a b && areUnifiable ltA ltB
areUnifiable RefTy{} _ = False
areUnifiable (FuncTy argTysA retTyA ltA) (FuncTy argTysB retTyB ltB)
  | length argTysA /= length argTysB
  = False
  | otherwise
...
replaceTyVars mappings t = case t of
  (VarTy key             ) -> fromMaybe t (Map.lookup key mappings)
  (FuncTy argTys retTy lt) -> FuncTy (map (replaceTyVars mappings) argTys)
                                     (replaceTyVars mappings retTy)
                                     (replaceTyVars mappings lt)
  (StructTy name tyArgs) -> StructTy (replaceTyVars mappings name)
                                     (fmap (replaceTyVars mappings) tyArgs)
