...
toC toCMode (Binder meta root) = emitterSrc
  (execState (visit startingIndent root) (EmitterState ""))
 where
  ...
  visit indent xobj = case obj xobj of
  ...
  visitStr' indent str i =
    ...
    appendToSrc
      (addIndent indent ++ "String *" ++ varRef ++ " = &" ++ var ++ ";\n")
    return varRef
  ...
  visitSymbol indent xobj@(XObj sym@(Sym path lookupMode) (Just i) t) =
    ...
    in  if isTypeGeneric t'
          then error
            ...
            if isFunctionType t' && not (isLookupLocal lookupMode) && not
                 (isGlobalVariableLookup lookupMode)
              then do
                let var = freshVar i
                appendToSrc
                  (  addIndent indent
  ...
  visitSymbol _ xobj@(XObj (Sym path _) Nothing _) =
    error ("Symbol missing info: " ++ show xobj)
  visitSymbol _ _ = error "Not a symbol."
  visitList :: Int -> XObj -> State EmitterState String
  visitList indent (XObj (Lst xobjs) (Just i) t) = case xobjs of
    -- Defn
    [XObj (Defn _) _ _, XObj (Sym path@(SymPath _ name) _) _ _, XObj (Arr argList) _ _, body]
      -> case toCMode of
        ...
        _       -> do
          ...
          when (name == "main") $ appendToSrc
            (addIndent innerIndent ++ "carp_init_globals(argc, argv);\n")
    ...
    -- Fn / λ
    [XObj (Fn name set) _ _, XObj (Arr argList) _ _, body] -> do
      let retVar            = freshVar i
      ...
      when needEnv $ do
        ...
        mapM_
          (\(XObj (Sym path lookupMode) _ _) -> appendToSrc
            (  addIndent indent
            ++ lambdaEnvName
            ++ "->"
            ++ pathToC path
      ...
      appendToSrc
        (addIndent indent ++ "  .callback = (void*)" ++ callbackMangled ++ ",\n"
        )
      appendToSrc
        (  addIndent indent
        ++ "  .env = "
        ++ (if needEnv then lambdaEnvName else "NULL")
        ++ ",\n"
        )
      appendToSrc
        (  addIndent indent
        ++ "  .delete = (void*)"
        ++ (if needEnv then "" ++ lambdaEnvTypeName ++ "_delete" else "NULL")
        ++ ",\n"
        )
      appendToSrc
        (  addIndent indent
        ++ "  .copy = (void*)"
        ++ (if needEnv then "" ++ lambdaEnvTypeName ++ "_copy" else "NULL")
        ++ "\n"
        )
    ...
    -- Def
    [XObj Def _ _, XObj (Sym path _) _ _, expr] -> case toCMode of
      Functions -> return ""
      _         -> do
        appendToSrc (addIndent indent ++ "{\n")
        let innerIndent = indent + indentAmount
        ...
        appendToSrc
          (addIndent innerIndent ++ pathToC path ++ " = " ++ ret ++ ";\n")
        delete innerIndent i
        appendToSrc (addIndent indent ++ "}\n")
        return ""
    ...
    [XObj Let _ _, XObj (Arr bindings) _ _, body] ->
      ...
      in
        do
          ...
          when isNotVoid $ appendToSrc
            (addIndent indent' ++ letBodyRet ++ " = " ++ ret ++ ";\n")
          delete indent' i
          appendToSrc (addIndent indent ++ "}\n")
          return letBodyRet
    ...
    [XObj If _ _, expr, ifTrue, ifFalse] ->
      ...
      in
        do
          let isNotVoid = ty ifTrue /= Just UnitTy
              ifRetVar  = freshVar i
          when isNotVoid
      ...
      -> let
           ...
           tagCondition var periodOrArrow caseTy (caseLhs@(XObj (Lst (XObj (Sym firstPath@(SymPath _ caseName) _) _ _ : caseMatchers)) caseLhsInfo _))
             ...
             -- This requires a bunch of extra machinery though, so this will do for now...
               [ var ++ periodOrArrow ++ "_tag == " ++ tagName
                   caseTy
                   (removeSuffix caseName)
               ]
               ++ concat
                    (zipWith
                      (\c i -> tagCondition
           ...
           removeOuterRefTyIfMatchRef t = case matchMode of
             MatchValue -> t
             MatchRef   -> case t of
               RefTy inner _ -> inner
           ...
           emitCase exprVar isFirst (caseLhs@(XObj (Lst (XObj (Sym firstPath@(SymPath _ caseName@(firstLetter : _)) _) _ _ : caseMatchers)) caseLhsInfo _), caseExpr)
             ...
             -- A list of things, beginning with a tag
               ...
               let refModifications = case matchMode of
                     MatchValue -> (".", "")
                     MatchRef   -> ("->", "&")
               appendToSrc
                 (  "if("
                 ++ joinWith
                      " && "
                      (tagCondition exprVar
                                    (fst refModifications)
                                    (removeOuterRefTyIfMatchRef exprTy)
                                    caseLhs
                      )
                 ++ ") {\n"
                 )
               ...
               zipWithM_
                 (emitCaseMatcher refModifications (removeSuffix caseName))
                 caseMatchers
                 [0 ..]
           ...
           emitCase exprVar isFirst (XObj (Sym firstPath _) caseLhsInfo _, caseExpr)
             ...
             -- Single variable
               ...
               emitCaseEnd caseLhsInfo caseExpr
           emitCase _ _ x = error ("Fell through: " ++ show x)
         ...
         in
           do
             ...
             appendToSrc
               (  addIndent indent
               ++ "  fprintf(stderr, \"Unhandled case in 'match' expression at "
               ++ quoteBackslashes (prettyInfo i)
               ++ "\\n\");\n"
               )
    ...
    XObj (Match _) _ _ : _ -> error "Fell through match."
    -- While
    [XObj While _ _, expr, body] ->
      ...
      in
        do
          ...
          delete indent' i
          appendToSrc
            (addIndent indent' ++ conditionVar ++ " = " ++ exprRetVar' ++ ";\n")
          appendToSrc (addIndent indent ++ "}\n")
          return ""
     where
      visitWhileExpression :: Int -> State EmitterState String
      visitWhileExpression ind = do
        s <- get
        let (exprRetVar, exprResultState) =
              runState (visit ind expr) (EmitterState "")
            exprSrc = emitterSrc exprResultState
        modify (\x -> x { emitterSrc = emitterSrc s ++ exprSrc })
        return exprRetVar

    -- Do
    XObj Do _ _ : expressions -> do
    ...
    -- Address
    [XObj Address _ _, value] -> do
      valueVar <- visit indent value
      return ("&" ++ valueVar)

    -- Set!
    [XObj SetBang _ _, variable, value] -> do
      valueVar <- visit indent value
      let
        properVariableName = case variable of
          (XObj (Lst (XObj (Sym (SymPath _ "copy") _) _ _ : symObj@(XObj (Sym sym _) _ _) : _)) _ _)
            -> "*" ++ pathToC sym
      ...
      appendToSrc
        ...
        ++ "; "
        ++ " // "
        ++ show (fromMaybe (VarTy "?") (ty variable))
        ++ " = "
        ++ show (fromMaybe (VarTy "?") (ty value))
        ++ "\n"
        )
      return ""

    -- The
    [XObj The _ _, _, value] -> do
    ...
    -- Ref
    [XObj Ref _ _, value] -> do
      var <- visit indent value
      let Just t' = t
          fresh   = mangle (freshVar i)
      if isNumericLiteral value
        then do
          let literal        = freshVar i ++ "_lit"
              Just literalTy = ty value
          appendToSrc
            (  addIndent indent
            ++ "static "
            ++ tyToCLambdaFix literalTy
            ++ " "
            ++ literal
            ++ " = "
            ++ var
    ...
    -- Deref
    [XObj Deref _ _, value] -> do
      x <- visit indent value
      return ("(*" ++ x ++ ")")

    -- Deftype
    XObj (Deftype    _) _ _ : XObj (Sym _ _) _ _ : _         -> return ""
    -- DefSumtype
    XObj (DefSumtype _) _ _ : XObj (Sym _ _) _ _ : _         -> return ""
    -- Template
    [XObj (Deftemplate _       ) _ _, XObj (Sym _ _) _ _   ] -> return ""
    [XObj (Instantiate template) _ _, XObj (Sym path _) _ _] -> case toCMode of
      Globals -> return ""
      _       -> do
        let Just t' = t
    ...
    -- Alias
    XObj (Defalias _)    _ _ :             _ -> return ""
    -- External
    XObj (External _)    _ _ :             _ -> return ""
    -- Macro
    XObj Macro           _ _ :             _ -> return ""
    -- Dynamic
    XObj Dynamic         _ _ :             _ -> return ""
    -- DefDynamic
    XObj DefDynamic      _ _ :             _ -> return ""
    -- Command
    XObj (Command   _  ) _ _ :             _ -> return ""
    -- Primitive
    XObj (Primitive _  ) _ _ :             _ -> return ""
    -- Interface
    XObj (Interface _ _) _ _ :             _ -> return ""
    -- Break
    [                        XObj Break _ _] -> do
    ...
    func@(XObj (Sym _ (LookupGlobalOverride overriddenName)) _ _) : args -> do
    ...
    func@(XObj (Sym path (LookupGlobal mode AFunction)) _ _) : args -> do
      ...
      if retTy == UnitTy
        then do
          appendToSrc
            (addIndent indent ++ funcToCall ++ "(" ++ argListAsC ++ ");\n")
          return ""
        else do
          let varName = freshVar i
          appendToSrc
    ...
    func : args -> do
      ...
      let
        funcTy = case ty func of
          Just actualType -> actualType
          _               -> error ("No type on func " ++ show func)
        FuncTy argTys retTy _ = funcTy
        castToFn              = if unwrapLambdas
          then
            tyToCLambdaFix retTy
            ++ "(*)("
            ++ joinWithComma (map tyToCRawFunctionPtrFix argTys)
            ++ ")"
          else
            tyToCLambdaFix retTy
            ++ "(*)("
            ++ joinWithComma (map tyToCLambdaFix argTys)
            ++ ")"
        castToFnWithEnv = if unwrapLambdas
          then
            tyToCLambdaFix retTy
            ++ "(*)("
            ++ joinWithComma
                 (map tyToCRawFunctionPtrFix
                      (StructTy (ConcreteNameTy "LambdaEnv") [] : argTys)
                 )
            ++ ")"
          else
            tyToCLambdaFix retTy
    ...
    [] -> do
      appendToSrc (addIndent indent ++ "/* () */\n")
      return ""
  visitList _ xobj@(XObj (Lst _) Nothing Nothing) =
    error ("List is missing info and type! " ++ show xobj)
  visitList _ xobj@(XObj (Lst _) Nothing (Just _)) =
    error ("List is missing info! " ++ show xobj)
  visitList _ xobj = error ("Must visit list! " ++ show xobj)
  createArgList :: Int -> Bool -> [XObj] -> State EmitterState String
  createArgList indent unwrapLambdas args = do
    argStrings <- mapM (visit indent) args
  ...
  unwrapLambda variableName ty =
    if isFunctionType ty then variableName ++ ".callback" else variableName
  visitArray :: Int -> XObj -> State EmitterState String
  visitArray indent (XObj (Arr xobjs) (Just i) t) = do
  ...
  visitArrayElement
    :: Int -> String -> Ty -> Int -> XObj -> State EmitterState ()
  visitArrayElement indent arrayVar innerTy index xobj = do
  ...
  visitStaticArray indent (XObj (StaticArr xobjs) (Just i) t) = do
    let
      arrayVar     = freshVar i
  ...
  visitStaticArrayElement
    :: Int -> String -> Ty -> Int -> XObj -> State EmitterState ()
  visitStaticArrayElement indent arrayDataVar innerTy index xobj = do
    visited <- visit indent xobj
    appendToSrc
      (  addIndent indent
      ++ arrayDataVar
...
delete indent i = mapM_ deleterToC (infoDelete i)
 where
  ...
  deleterToC FakeDeleter{} = return ()
  deleterToC PrimDeleter{} = return ()
  deleterToC RefDeleter{}  = return ()
  deleterToC deleter@ProperDeleter{} =
    appendToSrc
      $  addIndent indent
      ++ ""
      ++ pathToC (deleterPath deleter)
      ++ "("
      ++ mangle (deleterVariable deleter)
      ++ ");\n"

defnToDeclaration :: MetaData -> SymPath -> [XObj] -> Ty -> String
defnToDeclaration meta path@(SymPath _ name) argList retTy =
  let (XObj (Lst annotations) _ _) =
          fromMaybe emptyList (Map.lookup "annotations" (getMeta meta))
      annotationsStr = joinWith " " (map strToC annotations)
      sep            = if not (null annotationsStr) then " " else ""
  in  annotationsStr ++ sep ++ if name == "main"
...
templateToC template path actualTy =
  let mappings    = unifySignatures (templateSignature template) actualTy
      ...
      tokens      = concatMap
        (concretizeTypesInToken mappings (pathToC path) declaration)
        definition
...
defStructToDeclaration structTy@(StructTy typeName typeVariables) path rest =
  let indent = indentAmount
      ...
      typedefCaseToMemberDecl (XObj (Arr []) _ _) =
          sequence $ pure $ appendToSrc (addIndent indent ++ "char __dummy;\n")
      typedefCaseToMemberDecl (XObj (Arr members) _ _) =
          mapM (memberToDecl indent) (pairwise members)
...
defSumtypeToDeclaration sumTy@(StructTy typeName typeVariables) path rest =
  let
    ...
    emitSumtypeCase indent xobj@(XObj (Lst [XObj (Sym (SymPath [] caseName) _) _ _, XObj (Arr memberTys) _ _]) _ _)
      = do
        appendToSrc (addIndent indent ++ "struct {\n")
        let members = zipWith (\anonName tyXObj -> (anonName, tyXObj))
                              anonMemberSymbols
                              memberTys
    ...
    emitSumtypeCaseTagDefinition (tagIndex, xobj@(XObj (Sym (SymPath [] caseName) _) _ _))
      = appendToSrc
        ("#define " ++ tagName sumTy caseName ++ " " ++ show tagIndex ++ "\n")
  in
    if isTypeGeneric sumTy
      then ""
      else emitterSrc (execState visit (EmitterState ""))

defaliasToDeclaration :: Ty -> SymPath -> String
defaliasToDeclaration t path = case t of
...
toDeclaration (Binder meta xobj@(XObj (Lst xobjs) _ t)) = case xobjs of
  ...
  [XObj Def _ _, XObj (Sym path _) _ _, _] ->
    let Just t' = t in "" ++ tyToCLambdaFix t' ++ " " ++ pathToC path ++ ";\n"
  ...
  [XObj (Instantiate template) _ _, XObj (Sym path _) _ _] ->
    let Just t' = t in templateToDeclaration template path t'
  ...
  _                          -> error
    (  "Internal compiler error: Can't emit other kinds of definitions: "
    ++ show xobj
    )
...
paramListToC xobjs = intercalate ", " (map getParam (filter notUnit xobjs))
 where
  ...
  getParam (XObj (Sym (SymPath _ name) _) _ (Just t)) =
    tyToCLambdaFix t ++ " " ++ mangle name
...
binderToDeclaration typeEnv binder =
  ...
  in  case xobj of
        ...
        _                  -> case ty xobj of
          Just t -> if isTypeGeneric t
            then Right ""
            else Right (toDeclaration binder ++ "")
          Nothing -> Left (BinderIsMissingType binder)
...
envToDeclarations typeEnv env =
  let bindersWithScore = sortDeclarationBinders
        typeEnv
        (map snd (Map.toList (envBindings env)))
  in
    do
      okDecls <- mapM
        (\(score, binder) -> fmap
...
checkForUnresolvedSymbols = visit
 where
  ...
  visit xobj = case ty xobj of
    Nothing -> visitXObj
    Just t ->
      if isTypeGeneric t then Left (UnresolvedGenericType xobj) else visitXObj
   where
    visitXObj = case obj xobj of
  ...
  visitList (XObj (Lst xobjs) i t) = case mapM visit xobjs of
    Left  e -> Left e
    Right _ -> return ()
  visitList _ =
    error "The function 'visitList' only accepts XObjs with lists in them."
  visitArray :: XObj -> Either ToCError ()
  visitArray (XObj (Arr xobjs) i t) = case mapM visit xobjs of
    Left  e -> Left e
    Right _ -> return ()
  visitArray _ =
    error "The function 'visitArray' only accepts XObjs with arrays in them."
  visitStaticArray :: XObj -> Either ToCError ()
  visitStaticArray (XObj (StaticArr xobjs) i t) = case mapM visit xobjs of
    Left  e -> Left e
    Right _ -> return ()
  visitStaticArray _ = error
    "The function 'visitStaticArray' only accepts XObjs with arrays in them."
